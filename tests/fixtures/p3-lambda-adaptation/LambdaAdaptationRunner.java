...
public class LambdaAdaptationRunner {
 static void run(String name,java.util.concurrent.Callable<Object> call){LambdaAdaptationSupport.calls=0;try{System.out.println(name+":"+call.call());}catch(Throwable error){System.out.println(name+":"+error.getClass().getName());}}
 ...
 static void unboundCheck(String name,Object receiver,Object value){LambdaAdaptationSupport.calls=0;ToIntBiFunction f=LambdaAdaptationProbe.unbound();try{System.out.println(name+":"+f.applyAsInt(receiver,value)+":calls="+LambdaAdaptationSupport.calls);}catch(Throwable error){System.out.println(name+":"+error.getClass().getName()+":calls="+LambdaAdaptationSupport.calls);}}
 static void constructorCheck(String name,Object value){LambdaAdaptationSupport.calls=0;Function f=LambdaAdaptationProbe.constructor();try{System.out.println(name+":"+((LambdaAdaptationBox)f.apply(value)).value()+":calls="+LambdaAdaptationSupport.calls);}catch(Throwable error){System.out.println(name+":"+error.getClass().getName()+":calls="+LambdaAdaptationSupport.calls);}}
 public static void main(String[]args){
  ...
  unboundCheck("receiverNull",null,"text");
  unboundCheck("receiverNullWrongArg",null,Integer.valueOf(7));
  unboundCheck("receiverWrong",Integer.valueOf(7),"text");
  unboundCheck("receiverWrongNullArg",Integer.valueOf(7),null);
  unboundCheck("receiverValidNullArg",new LambdaAdaptationSupport(),null);
  unboundCheck("receiverValidWrongArg",new LambdaAdaptationSupport(),Integer.valueOf(7));
  constructorCheck("constructorText","text");
  constructorCheck("constructorNull",null);
  constructorCheck("constructorWrong",Integer.valueOf(7));
 }
}
