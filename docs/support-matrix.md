...
- **NotValidated**：实现或依赖可能在该环境工作，但 P0 没有建立支持证据；按不支持处理。
- **Unsupported**：入口明确拒绝，或超出 P0 支持范围。
...
| Nested JAR/ZIP candidate | MetadataOnly | NotImplemented | NotImplemented | NotImplemented | `candidate_not_scanned` | P0 只标记候选，不递归。nested 读取与 Boot layout 属 P1。 |
...
- **output-level：** 仅物理 entry、Header、所选方法原始 instruction/handler facts 及 coverage/execution/diagnostics；没有 Java、伪代码或 runtime view。
...
| JSON CLI | stdin / `--request FILE` | Supported | 单请求、1 MiB 控制面；支持 elapsed budget，但不暴露 cancellation token 注入。 |
...
预算约束累计的 input/archive-entry/entry/read/class/attribute/code/result/output/elapsed 十个维度，但不构成进程 RSS、硬 deadline、恶意并发写入下完整文件瞬时一致性或任意规模 ZIP 的保证。
