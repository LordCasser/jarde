...
- [2026-09-26 持续语法巡查](evidence/java-syntax-2026-09-26/)：自写方法引用/精确重抛/多资源 TWR/char switch 场景的三方对照定位出四项缺口，独立交付为 [精确重抛](changes/recover-precise-rethrow/tasks.md)（`finally_copy` 候选缺 `catch_type==0` 前置，具名行用户 catch 被整方法拒绝）、[多资源 TWR](changes/recover-multi-resource-twr/tasks.md)（层级几何误用内层 handler span 末，`RangeEnd`）、[SAM 装箱适配](changes/recover-boxed-sam-adaptations/tasks.md)（impl 基本类型 ↔ instantiated 包装类的逐槽配对未证明，含 `int[]::new`）；char 选择表达式为 `int` 局部时的键拼写归 2c.12/局部类型家族，不开新 change。jadx 基线偏离：preciseRethrow 的 `throws` 宽化为 `Exception`、多资源 TWR 展开为手写 close 链。第二批（assert/do-while continue/嵌套三元/多捕获）全部归属已登记 change，无新 change；冻结输出中的多捕获 catch 空体缺口已由 [umbrella 2.2](changes/present-proved-java-structure/tasks.md) 修复，保留修前证据供回放。
