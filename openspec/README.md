...
当前交付仅包含文档。P0–P5 的 proposal、design、specs 和 tasks 已建立；实现与行为验收均未完成，不发布引擎能力或性能承诺。
...
后续明确进入实现时，按阶段运行 `$openspec-apply-change <change-name>`。已有 `.agents/skills` 为 OpenSpec 生成的工作流文档，不属于引擎代码。当前不执行 apply、archive 或发布。
