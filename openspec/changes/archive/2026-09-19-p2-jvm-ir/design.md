...
- **保留的指令操作数（0.2 已交付，D03/D27）**
- **映射 totality 由版本钉死保证，不是 `debug_assert`**：`wide` 在 JVMS 6.5 里只能包裹 12 个 opcode，noak `=0.7.0` 对这 12 个以外直接报 `InvalidInstruction`（适配层因此不会看到未映射形态）；`debug_assert` **只在 debug/测试构建**下把映射遗漏判为失败。升级 noak 时必须重新确认这条前提（3.1 的升级门槛），不得把断言当作 release 保证。：`newarray` 的 atype、`multianewarray` 的 dimensions、`invokeinterface` 的 count 与 **wide 的 effective opcode** 都在 crate-private 的 `InstructionOperands` 上保留（见上面的字段），由共享 noak 事件适配直接给出，**不另写 decoder**。
  ...
  - **`multianewarray` 的 `stack_delta` 在 3.3 保持 `None`**（与 `athrow` 同类：仅靠 opcode 定不了弹栈槽数），由 **4.1** 按 `1 - dimensions` 关闭；0.2 不承担这条。
  - **夹具必须显式给出编码事实**：`InstructionOperands::default()` 的 `effective_opcode` 是 `0x00`（不是原始 opcode），漏设会静默退化成 `nop` 语义。
...
- **事实的消费者必须把该事实写进自己的 `requires`**：否则 invalidation 检查对它不生效。原 3.4 读取 `raw.effects` 却未声明 `Effects`，0.3 已补齐并用 Effects 为 stale/未产出的反例验证入口拒绝。成功发布阶段时必须保留实际 facts 供下一 pass 使用；不能只在 ledger 标记存在而丢掉 payload。
...
- **`jsr` 所在块与续块按「包含该 BCI 的块」查**，不按「块起始 BCI 相等」查：`jsr` 可以出现在块中间（前面是同块的普通指令），此时它的续块仍是**该 jsr 所在块**的后继。BCI→块的查法在 `cfg` 与 `call_context` 之间必须是同一种（`partition_point`：最后一个起始 ≤ bci）。用精确相等（`binary_search_by_key` + 失败即 `continue`）会在 `jsr` 非块首时静默丢掉这条关系，使**活调用点的续块被真值表判为不可达**，进而让 3.4 跳过「可达子程序体无 `ret`」的拒绝、对非法字节码报 `Established`，并把活调用点误列为 `unreachable_call_sites`。3.3 的既有证据（ECJ 语料与 `jsr_returns_stay_unresolved_…`）恰好都把 `jsr` 放在块首，所以该缺陷零覆盖；修正必须配一条**非块首 `jsr`** 的 3.3 回归与一条 3.4 回归（ECJ 45–48 的 `unreachable` 在此修正后由 `[8,11,15]` 变为 `[11,15]`，这才是 JVMS 语义：`ret` 返回到 8）。
  **修正面已由父级审计为单点**（0.4 照此做，不必重新全仓排查）：受影响者**只有** `cfg::jsr_continuations`（`from` 用精确匹配，`jsr` 非块首时整条 `continue`，续块因此丢失）；`RawCfg::unresolved_returns` 由 `jsr_call_sites` 直接扫描指令构造，**不受影响**，不得顺手改；`RawCfg.edges` 的端点（`block_position(edge.from_bci/to_bci)`）与指令级查找（按 BCI 查指令）用精确匹配是**正确**的——边端点按构造恒为块首（分支/switch/handler 目标与块结束者的后继都被 `leader_flags` 标为 leader），指令 BCI 唯一，两处都不动。0.4 只需把 `jsr_continuations` 的 `from` 换成 `call_context::block_of` 同款读法（`partition_point` 取最后一个起始 ≤ bci），并让两种查法合并为一处。
...
- **`wide` 缺口已修正**：0.2 已保留 wide load/store/ret 的有效 opcode，并同步分块、终结指令与 effects；后续沿用该事实，不重复新增 decoder。
...
- **最小实现**：复用 reader operands 和现有 effects，只建立返回地址证明需要的状态与工作列表；不增加 pass 框架、不复制 decoder、不提前实现完整 4.x Frame。可以先限制支持形态，只要范围明确、合法历史 finally 对照仍成立、不支持时保守停止；不能让启发式结果穿过 Established 边界。
- **共享与嵌套**：共享子程序分别按调用上下文证明，同一原始 `ret` 可以对应多个已证明的调用点。嵌套 continuation 仅在内层返回成立后继续；内层写入必须影响外层 locals 的有效性。`affected_locals` 是含嵌套及 handler 的 may-write 集，不是返回时刻的值，不能忽略内层对外层地址槽的覆盖。
- **异常输入**：按 throw-site、handler ordinal 与 active context 传播该点 locals；handler 清空原栈并压入异常 reference，不能继承栈顶返回地址。无法判定归属或异常合流时明确 fallback；不能只保存异常覆盖 ordinal 就声称返回值流完整。
- **可达范围与载荷**：现代 51+ 的 legacy opcode 按整个可靠前缀检查 dialect，含死代码和 wide ret。上下文分析对可达调用点要求返回点已解码；死调用点若返回点未解码，不生成 context、也不阻塞活调用点。`contexts.len()` 等于前缀中**返回点已解码**的 jsr/jsr_w 站点数；`returns.len()` 等于前缀内 ret 数。死 ret 的 `targets=[]` 不授权 3.5 建边，截断前缀不授权发布完整 CanonicalCFG。
- **计费与取消**：延续 0.3/0.3b 的 `IrItems`/`IrEdges`/`AnalysisSteps` 与装配检查点；新增 token、状态槽、逻辑输入、工作列表增长先计费。零/恰好/超限及中途取消均不得发布半初始化 CallContexts，失败使用原请求已保留的 raw facts。
- **公开状态**：返回地址无法证明用 `ir_call_context_unresolved`，不把阶段标 Completed；预算/取消保留各自原因，图与事实内部矛盾才用 inconsistent。`verification` 仍为 NotPerformed，P2 representation 始终 Bytecode。现有 driver 在 `Established(_contexts)` 分支丢弃载荷，尚无下游消费者；3.5 必须把它保留在同一 run 中并实际传给规范化，不能只让 ledger 声称 fact 存在。
- **3.4b 验收**：本轮 null、pop 后替换、aload 地址、嵌套覆盖四个反例均不得通过；合法直线/嵌套、共享历史 finally、handler 回接、wide/版本与死代码对照仍须有真实字节证据。覆盖不同 throw-site 的 locals、预算/取消和最后有效阶段；证明旧规则失败，再独立复核。原 3.4 已有用例保留为回归，不反复搬迁文件或重做依赖选型。
- **实现注记（2026-09-18 复核后补，避免后人误读为「有测试保护」）**：
  - **两处防御语句不可证伪**：录制循环里 `is_astore` 命中后的 `break`、以及不动点中 `Exception` 边贡献 `false` 的那一支——实测把任一处改成等价写法，全套测试仍绿（前者因为紧随的「非保留指令即终止」在同一指令处已生效；后者因为任何可能抛异常的块都含非保留指令，其出状态本来就是 `false`）。它们是**冗余防御**，不是被覆盖的行为。
  - **`keeps_stack_top` 刻意取最小集**：只承认 `nop`/`iinc`/`goto`/`goto_w`（真正不动栈顶）。`dup` 族在语义上仍把 token 留在栈顶，但被本实现判为「扰动」→ **已知误拒**（方向安全：多拒不错收）。编译器不产出该形态，故不为此放宽状态。
  - **一条输入假设**：状态以「入口块是唯一真来源」推得，其成立依赖「region 内不存在源在 region 之外的入边」。理论上只有「异常表项覆盖体外块、handler 在体内被正常流程到达」会破坏它；未构造出真实字节，故按假设记录而非按代码隐含。`dominates` 的 `barrier == entry` 捷径同源。
...
- **4.1 的边界与专用停止码**：初始化转换（`uninitializedThis`/new-site 的别名翻转、构造调用及其异常后继）与 **handler 入口状态**同属 **4.2**，4.1 只落实状态区别：`new` 压入带 new-site 的未初始化引用，构造器入口的 local 0 是 `UninitializedThis`，未初始化值**只能被纯栈操作搬动**（`astore`/`aload`/`dup*`/`pop*`/`swap`）；**异常边进入的块也不得凭空造入口状态**（canonical 的异常边聚合了若干 throw site，由它编出一个状态正是设计禁止的「一个状态代表多个 BCI」）。
  - 两处的停止码都是 **`ir_frame_deferred`**（Warning、`Partial`、不发布 `Frames`、保留 raw 与 canonical），**诊断消息必须点名命中的是哪一种**（未初始化值 vs 某条 handler 记录）。它表示「**本 build 证不出**」，与表示字节码自相矛盾的 `ir_frame_inconsistent` **必须分开**。**4.2 落地后该码依然存在，且是长期的**（不再是「4.2 未落地」的标志）：触发集合是「未初始化 token 被当作已初始化引用使用」（`getfield`/`athrow`/`areturn`/`ifnull`/`checkcast`/`instanceof`/作调用参数）、「不适用的 `<init>`」、「receiver 已是已初始化引用的 `<init>`」、以及「名为 `<init>` 却不是 `invokespecial` 的调用」——四者同一原因：**本层没有定义这条转移，所以不给帧**。**不得**因为「4.2 已完成」就把这些停止删掉。这与 `ir_legacy_normalization_unbounded` 同类：都是分阶段的实现边界码，不得用于掩盖真实不一致。
  - 码名刻意不写 `uninitialized`：两个触发点同属 4.2、同时退休，用一个按**原因**命名的码会让 handler 入口那条读起来像初始化问题。
  - **稠密表的类别正确性必须由逐行用例守，不能靠与 `cfg` 的交叉校验**：那条交叉校验只比**槽位深度**与 `ends_block`，而「弹两个 `Int`」与「弹两个 `Ref`」深度相同——4.1 因此漏掉了 `if_acmpeq`/`if_acmpne`（被并进 `if_icmp*` 的范围）与 `lshl`/`lshr`/`lushr`（弹出顺序写成 JVMS 散文顺序而非栈顺序：移位量是 `int` 且在栈顶）两处，症状都是**把合法 body 报成 `ir_frame_inconsistent`（Error）**，即把实现缺陷说成字节码矛盾，同时**反向放行**非法 body。凡「错误的行与原行槽位和相同」的族都是盲区，必须按族补逐行类别断言（而非铺满 256 条）：**数组加载/存储、转换、返回、引用单例、栈形式、局部访问、比较/分支、移位**。
  - **opcode ↔ 常量池种类的配对检查是本层的职责，且必须两侧一致**：字段侧（`getfield`/`putfield`）已只接受 `FieldRef`；方法侧曾对所有 invoke opcode 一律接受 `{MethodRef, InterfaceMethodRef, InvokeDynamic}`，使 `invokevirtual` 指向 `InvokeDynamic` 这类非法配对**多弹一个 receiver** 后放行。本层不是 verifier（`verification` 恒 `NotPerformed`），但既然字段侧已承担该配对检查，只做一半会让读者无法判断哪一半是有意的；按 JVMS 6.5 + SE 8：`invokevirtual`→`MethodRef`；`invokespecial`/`invokestatic`→`MethodRef` 或 `InterfaceMethodRef`；`invokeinterface`→`InterfaceMethodRef`；`invokedynamic`→`InvokeDynamic`。  - **判定线（写下来，免得后人把有意的一半当漏做）**：本层**只在「不读该常量池条目就说不清栈形状」时才读它**，并且读到了不匹配的种类就拒绝——因为那不是「非法输入」而是「推不出形状」。据此：`getfield`/`putfield` 要用字段描述符（读，且必须是 `FieldRef`）；`invoke*` 的 receiver 数来自 opcode、参数与返回类型来自条目（读，且种类必须与 opcode 配对）；`checkcast` 压的类型就是条目说的类（读，且必须是类类型）；**`instanceof` 则整行不读池**——它的形状恒为「弹一个引用、压一个 `int`」，条目的种类不影响形状，故 `aconst_null; instanceof #7`（`#7` 是 `Utf8`）会得到完整的帧表而**不被拒绝**。这是**有意**的：操作数合法性属 verifier 的职责，而本层不是 verifier（`verification` 恒 `NotPerformed`）。若将来 P5 需要操作数级校验，那是**新增一层能力**，不是补 4.1 的漏。
