...
4. **呈现为一条头。** 源代码是一条 `try (a; b)`，恢复文本同形；每层的 close 证据仍逐层验证（`close_of_level`/`close_handler`/`Suppressed` 原样），正常路径逆序 close 的既有证明复用。资源名沿用 `names` 拼写。
   冻结 release-8 类在 2.3b 后曾于 BCI 0 报 `jre_guard_span`，尽管 `new@1` 已接受两个构造站点：`report.rs` 当时先运行 `region::recover`，之后才运行 `field::plan`、`concat::plan` 与 `init::sites`。现已将这些不依赖 Region/局部命名的规划前移，再把只读 `Sites` 传入 Region/Guard。Guard 仅在初始化 Store 所读的 SSA 值与一个站点的构造结果一致、站点完整指令集合正好覆盖对应资源初始化且没有额外效果时扩展该资源范围。每个资源分别证明，整条头原子领取；站点缺失、身份不合或范围不完整不获构造站点路径授权。这只调整既有事实的传递顺序与归属，不增加机制，也不在 Guard 复制 `new@1` 的验证器。冻结正例已输出两资源头、体内返回和完整 BCI 来源；缺站点与错配站点的专门负例仍需补齐，故 2.4 验收暂不勾选。
