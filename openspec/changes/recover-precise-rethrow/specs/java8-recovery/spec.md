...
异常表某具名类型行（`catch_type != 0`）的 handler 体以重抛该 handler 入口存储的同一局部结尾时，恢复结果 SHALL 把该行呈现为普通用户 `catch` 子句，子句体末尾是重抛该子句参数的 `throw` 语句，且该子句之前的语句（如调用）按既有规则呈现。finally 拷贝候选检测 MUST 仅考虑 `catch_type == 0` 的行；具名类型行 MUST NOT 因 finally 拷贝候选被拒绝。呈现 MUST 保留 handler 入口存储与重抛 `athrow` 的物理来源。
...
#### Scenario: A handler reached by more than one row is out of scope

- **WHEN** 同一 handler 入口被多条异常表行到达（多捕获编码），其中含具名行
- **THEN** 该形状 SHALL 保持今天的处理（引用或既有呈现），MUST NOT 声称本形状的 `throw e;` 拼写
