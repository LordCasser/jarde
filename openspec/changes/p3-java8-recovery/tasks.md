...
- [ ] 1.1 定义 RecoveryProfile、模式前置条件、rule version、representation、quality、compile_status、semantic_validation、verification 和 fallback 类型；明确由 `jarde-jvm` 拥有、恢复侧消费的只读 method/CFG/value/effect/origin 输入，不公开全部可变中端或反向依赖门面；用真实 P2 结果消费与未满足前置条件 fixture 验证边界和不误识别
