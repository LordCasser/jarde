...
系统 SHALL 为恢复的声明、表达式、语句、异常路径和降级片段提供 origin set，能够映射到原始 BCI/attribute/CP 位置；派生 accessor/lambda 映射 MUST 标注派生关系和覆盖范围。

#### Scenario: Accessor-derived field expression

- **WHEN** Java 输出把 accessor 调用呈现为外层字段访问
- **THEN** 映射同时包含调用 accessor 的原始 BCI 和 accessor 访问字段的 BCI，并标记为 derived
