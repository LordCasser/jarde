...
`while (a > 0 && b > 0)` 的字节码是头部一条分支链：每个假边都离开循环，最后一条的真边进体。`||` 对偶：每个真边都进体，最后一条的假边离开。这些字节码不唯一对应作者的源代码：`while (true)` 加条件 `break` 也可能得到相同控制流；选择复合条件是语义保真的简洁规范化，不是原句法证明。本次 `LoopBool` 对照中 JADX 把 `andWhile` 写成复合 `while`，把 `orWhile` 写成 `while (true)` 加退出判断，故后者是超越 JADX 的输出质量机会，前者首先是追平其保真性。混用两种算子、任一条件证不了、或某条分支两边都不离开链时保持整段引用——优先级与括号没有字节码事实支撑。条件表达式为此增加 `&&`/`||` 两个二元操作，在已经证明的循环测试位使用；`do-while` 的闩锁同样适用（`doWhileComplex` 的链在闩锁上）。
