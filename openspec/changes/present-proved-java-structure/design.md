...
普通方法内的 `Join.mixedExits` 是另一个更窄的边界：case 1 `goto 40`，case 2 直接 `ireturn`，default 落入 40，40–41 是 switch 之后唯一一次 `iload; ireturn`。定向红测纠正了静态推断：提前返回使 `immediate_post_dominator(switch)` 为 `None`，当前 `switch_region` 没有 join 边界，case 1 与 default 都走入 40 并触发重叠拒绝。先从已有 normal-flow CFG 证明一个局部前向汇合候选：至少两个不同 case 入口的正常路径到达它，提前终止的臂可不经过；其普通入边受 switch 支配且没有外部入边或回边，遍历与入边检查均按预算计费。再复用 `Frame::switch_arm` 的 join 早期边界，让每条真实 case 臂在 40 前停止，40 由 switch 后续区域唯一认领。其它 case 入口只有在穿透已获证明时才作为臂边界；未证明穿透时不能早停后误写成 `break`。保留 overlap/覆盖检查；有回边、额外入边、跨臂跳转或未证明前向汇合时拒绝，不从 BCI 大小推导所有权。
...
`while (a > 0 && b > 0)` 的字节码是头部一条分支链：每个假边都离开循环，最后一条的真边进体。`||` 对偶：每个真边都进体，最后一条的假边离开。这些字节码不唯一对应作者的源代码：`while (true)` 加条件 `break` 也可能得到相同控制流；选择复合条件是语义保真的简洁规范化，不是原句法证明。冻结 `LoopBool` 对照中 JADX 把 `andWhile` 写成复合 `while`，把 `orWhile` 写成 `while (true)` 加退出判断，故后者是超越 JADX 的输出质量机会，前者首先是追平其保真性。混用两种算子、任一条件证不了、或测试链的体入口/出口并不唯一时保持有引用的原路径，不猜优先级与括号。`BinaryOp::LogicalAnd/LogicalOr` 及 Java 优先级已经由已证明的布尔值链提供；这里复用 AST，另证循环 CFG，不能因为表达式可拼就发布循环。

最小结构变更是在现有 `Region::Loop` 的测试位表示按求值顺序排列的一组测试，而非新增一种区域或照搬 `If` 合并器。头测循环从 header 到测试链，再到唯一体入口或精确出口；每个非首测试必须只有前测试这一正常流入边。每块验证分支操作数、条件用到的值及额外效果，并按真实 taken/fallthrough 边记录该叶应取的极性；扫描逐块经过现有预算/取消检查，失败不能留下半个已认领循环。覆盖和 source map 要逐块逐 BCI 结算。`Region::blocks`、Builder 的条件构造与 `for` 投影都必须读这组已证事实，尤其不能只把新条件文本拼在旧 `test_bci` 上而漏掉其它测试块。体内额外出口继续由已有 `LoopBreak` 负责。先落地头测形状并完整回归，再在同一证明约束上接闩锁形状；3.3 与 3.3b 分开验收，不能以头测通过勾选闩锁。

闩锁测试与头测共享短路真值关系，但有不同的循环拓扑：体至少执行一次。冻结 `DoLoopBool` 的 BCI 2、5、8 是体内增减，BCI 11 的 `iload` 与 BCI 12 第一测试却仍在同一个 canonical 块；没有一条“体尾块 → 第一测试块”的物理边。`andDo` 的 BCI 12 假边到 19 出口、真边到 BCI 15 第二测试，BCI 16 真边回到体入口 BCI 2，所以只有一条回边；`orDo` 的 BCI 12 与 16 的真边均回到 BCI 2，只有末测试的假边到 19，故自然循环可有两个 latch。旧 `latch_tested_loop` 仍只处理单测试；复合形状在它之前由 `latch_test_chain` 证明，并复用 `Loop` 的有序测试列表。当前准入子集要求体与第一测试同一 canonical 块，体前缀只含 `iinc`，其后值/分支后缀可完整呈现为测试；其余测试块只含条件值、仅由前一测试进入。链的每个完成边必须到同一体入口或出口，除初始入口外没有额外入边，异常边不得改变该结论。第一块由 body 物理认领一次，测试列表只记录条件来源，保持 `do-while` 在体后求值；不把前缀搬入条件，也不让 `Region::blocks` 重复认领。其他体形、混合布尔链、测试间额外效果和 `continue` 带来的第二测试入口仍拒绝。JADX 1.5.6 对 `andDo` 拆成 `do` 内 `break`，对 `orDo` 写 `while(true)` 加退出；Jarde 对冻结样例恢复复合闩锁，三方重编执行相同。冻结源码、字节码和历史基线见 `tests/fixtures/proved-java-structure/loop-boolean-do/` 与 `openspec/evidence/java-syntax-2026-09-26/loop-boolean-do/analysis.md`；当前回归在 `tests/p3_loop_boolean_do.rs`。

本地 JADX `2fb1b163` 的 `LoopRegionMaker.makeLoopRegion` 从循环出口块选一条 `IF` 作条件，`process` 再用 `IfRegionMaker.buildIfInfo/searchNestedIf` 尝试合并；`mergeNestedIfNodes` 对回边后继有提前拒绝，并要求下一测试满足其前驱/内联条件。这提供了「先确定测试，再考虑合并」的实现顺序，却不能作为复合闩锁的充分证明：`DoLoopBool.orDo` 的两条回边与 `andDo` 首测试前的体效果都必须保留，且 JADX 在两个冻结样例上实际没有输出复合 `do-while`。Jarde 复用其有序测试思路，但以完整正常流边、第一块的体/测试指令范围和物理块所有权为准，不从某一个所选 `IF` 或单一 `LoopInfo.getEnd()` 推出整个闩锁语法。上述代码条件是静态实现观察；具体失败环节若无 JADX 内部 CFG 跟踪，不据此断言。

这条循环恢复还有先于拼写的语义门禁。冻结 `LoopBool.andWhile` 的 BCI 3、7 都把假边送到 BCI 23 出口；现有 `header_tested_loop` 只把 BCI 3 作为头测试，BCI 7 被普通单臂 `if` 吸收，指向 BCI 23 的空臂没有 `break`，产物是会在 `(1,0)` 上无限循环的 `while (a>0) { if (b>0) { ... } }`。`covers` 检查自然循环内块的覆盖，不检查每条退出边的语义是否仍在。优先修复退出边归属：内部分支的精确循环出口要么投影为已有 `LoopBreak`，要么等整条复合测试链证明后由头部条件覆盖；其余形状拒绝，不能先产生 `Structured` 的错误程序。这个门禁属于区域/控制流，不应在 emitter 拼字符串时补救。[三方反例](../../evidence/java-syntax-2026-09-26/loop-boolean-exit/analysis.md)。

JADX 的 `LoopRegionMaker.process` 先经 `IfRegionMaker` 合并条件块，再把 `LoopInfo.getExitEdges()` 分为条件块已经表示的出口与额外需要 `insertLoopBreak` 的出口。可借用这个“每条退出边都必须有语法所有者”的核账方式；Jarde 的现有 Region、LoopBreak、正常流视图已足够表达所需事实，不移植 JADX 的可变 region 标记或其启发式合并。

值位的界线另由 `ShortCircuitValue` 的 CFG/SSA Phi 证明划定：两个唯一生产叶是整数 1/0，消费位按 Java 描述符确定为 `boolean`，分支测试和求值顺序也都已封闭时，结果不必先写成整数三元、再用 `% 2 != 0` 转回布尔。该证明中 `c ? true : false` 可写 `c`，`c ? x : false` 可写 `c && x`，`c ? true : x` 可写 `c || x`；反极性先按分支边取反，再在 AST 中组合，不能交换有副作用的两项。只在这种已证明的值消费位使用逻辑运算，不把普通语句位的两层 `if` 折叠（任务 1.4 的非目标仍成立）。同一字节码可能对应多种源码拼写，因此选择逻辑表达式是确定的语义规范化，不宣称还原原作者的精确句法。`BinaryOp` 复用已有二元节点并补布尔类型与 Java 优先级；`build_short_circuit_statement` 的 `Return` 路径先做这项保真构造，其它消费者后续另做。原始生产叶不是严格 1/0、消费位是 `int`、或 CFG/SSA/类型证明任一失败时，不执行规范化，保留既有引用或低位转换。JADX 的 `IfRegionMaker.mergeIfInfo` 通过条件边的共同出口选 AND/OR，可借鉴极性推导；Jarde 不照搬其区域合并，而沿用自身已封闭的 Phi 证明，避免在语句位误折叠。[三方证据](../../evidence/java-syntax-2026-09-26/boolean-short-circuit-return/analysis.md)。

后续消费位不需要新增 Region：`prove_short_circuit_value` 已分别要求静态/实例字段的 `Z` 描述符、`boolean[]` 元素、静态调用的 `(Z)V` 以及局部的布尔声明决定，并证明每个消费者唯一。任务 2c.17b 将严格 1/0 叶投影移到 `build_short_circuit_statement` 的消费者分派之前；字段的 `field_value` 接受已呈现的 `Type::Boolean`，数组、调用与局部不再把它重包成低位转换。实例字段接收者以及数组引用和下标仍由原证明与构建顺序先行求值；只改变右值的语法树，不搬移这些表达式。非 0/1 叶、未证类型和额外消费者走原路径。

混合链 `(a && b()) || c()` 的物理 `c()` 块由两条边共享；把 CFG 展开成整数三元 AST 后再递归化简，会把这个有副作用的块复制进两个文本分支，不能靠表达式文本相同就合并。已落地的窄投影只在现有 `ShortCircuitValue` 证明成功后，核对恰好三个测试的 canonical 边：首测试分别到第二测试和同一个尾测试，第二测试到尾测试或一个 1/0 叶，尾测试到两个生产叶；纯跳转 gateway 先按已证目标解析。按每条实际边的极性构造 `(root && second) || tail` 或 `(root || second) && tail`，每个测试只出现在最终表达式一次。形状不匹配则保留原整数树的处理，不猜共享性，也不增加通用 CFG matcher。冻结的[字段](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-field/)、[实例字段](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-instance-field/)、[数组](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-array/)、[调用](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-argument/)和[局部](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-local/)证据与 Runner 检验此投影；JADX 的逻辑文本是对照，执行与调用次数仍以原 class 为准。
