...
闩锁测试与头测共享短路真值关系，但有不同的循环拓扑：体至少执行一次。冻结 `DoLoopBool` 的 BCI 2、5、8 是体内增减，BCI 11 的 `iload` 与 BCI 12 第一测试却仍在同一个 canonical 块；没有一条“体尾块 → 第一测试块”的物理边。`andDo` 的 BCI 12 假边到 19 出口、真边到 BCI 15 第二测试，BCI 16 真边回到体入口 BCI 2，所以只有一条回边；`orDo` 的 BCI 12 与 16 的真边均回到 BCI 2，只有末测试的假边到 19，故自然循环可有两个 latch。现有 `latch_tested_loop` 的 `latches.len()==1` 和 `Loop` 单测试字段都不足以表达后一形状。推广时先按同块指令次序证明体前缀恰执行一次且在第一测试值之前；该块只能由 body 物理认领一次，第一测试只拥有它的值/分支后缀，其余测试块必须纯、仅由前一测试进入。所有短路完成边只到同一体入口或出口，并保持 `do-while` 在体后求值的位置；不能为了套用单 latch 把体效果移入条件。现有一块 `do-while` 的 body Straight 与测试共用块的处理可作接缝，但需要额外的复合链/来源核账，不能让 `Region::blocks` 重复认领。JADX 1.5.6 对 `andDo` 拆成 `do` 内 `break`，对 `orDo` 写 `while(true)` 加退出，这两例说明完整的闩锁投影会改善其语法质量。冻结 fixture 的源码、字节码和执行对照见 `tests/fixtures/proved-java-structure/loop-boolean-do/`；证据已固定，任务仍待实现。
