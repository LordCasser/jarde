...
普通方法内的 `Join.mixedExits` 是另一个更窄的边界：case 1 `goto 40`，case 2 直接 `ireturn`，default 落入 40，40–41 是 switch 之后唯一一次 `iload; ireturn`。当前 `switch_region` 已从 normal-flow post-dominator 得到 40，却在没有 case 穿透时为各臂使用普通 `Frame::arm`；该 frame 只把 40 设为通常 boundary，没有设置 `switch_join`。`region_at_inner` 在检查通常 boundary 前就把 40 插入全局 `visited`，下一臂到达 40 时被误判为重入，最后触发所有权拒绝。这里复用已有 `Frame::switch_arm` 的 join 早期边界：每条真实 case 臂都在共享 join 前停止，40 留给 switch 后续区域唯一认领；终止于 `return` 的臂无需经过 40。其它 case 入口只有在穿透已获证明时才作为臂边界；未证明穿透时不能早停后误写成 `break`。穿透探测仍决定 case 排列及是否发 `break`。保留 overlap/覆盖检查；有回边、额外入边、跨臂跳转或未证明前向汇合时拒绝，不从 BCI 大小推导所有权。
