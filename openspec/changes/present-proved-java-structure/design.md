...
后续消费位不需要新增控制流机制：`prove_short_circuit_value` 已分别要求静态/实例字段的 `Z` 描述符、`boolean[]` 元素、静态调用的 `(Z)V` 以及局部的布尔声明决定，并证明每个消费者唯一。任务 2c.17b 将严格 1/0 叶投影移到 `build_short_circuit_statement` 的消费者分派之前，继续复用 2c.17a 的 AST 转换；字段的 `field_value` 接受已呈现的 `Type::Boolean`，数组、调用与局部不再把它重包成低位转换。实例字段接收者以及数组引用和下标仍由原证明与构建顺序先行求值；只改变右值的语法树，不搬移这些表达式。非 0/1 叶、未证类型和额外消费者走原路径。冻结的混合短路类目前均结构化但仍写数值三元，JADX 已写逻辑表达式；[字段](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-field/)、[实例字段](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-instance-field/)、[数组](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-array/)、[调用](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-argument/)和[局部](../../evidence/java-syntax-2026-09-25/mixed-short-circuit-local/)证据与 Runner 可直接用来检验。
