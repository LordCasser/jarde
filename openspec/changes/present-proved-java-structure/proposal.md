...
- 已由 `ShortCircuitValue` 的 CFG/SSA Phi 和严格 1/0 生产叶证明的布尔值链，在 `return` 之外的已证布尔字段、布尔数组、`(Z)V` 调用参数和布尔局部消费位，也 MUST 保持 `&&`/`||` 的求值顺序；接收者和下标的原求值顺序不能被右值投影改变。不新增 Region 或条件匹配器。
