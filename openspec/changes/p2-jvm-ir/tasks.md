...
P1 与验证维护均已归档。原任务 1.1–1.3、2.1–2.5、3.1–3.3 共 11 项有交付记录；新增 0.1–0.5 后为 **15/25**（0.1、0.2、0.4、0.5 已完成并各自独立复核 Approve、CI 全绿并记录 run；0.3 进行中）。3.4 的工作区候选**已实现但未验收**——它阻塞在 0.3（派生存储计费）与 R2/R3（`ret` 值流、异常路径 locals）上，且 3.1 的 CI 记录与其未提交状态不构成背书。当前顺序：**0.3 → 3.4（按新契约重写值流与异常上下文）→ 3.5 → 4.x → 5.x**。已勾选不表示本轮发现的缺陷已消失。
