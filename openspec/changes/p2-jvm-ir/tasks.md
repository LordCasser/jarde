...
- [ ] 0.3b 修正 3.4 派生存储的分配前 `IrItems` 计费、实际边/工作列表计费与装配期取消检查：照 design 3.4 的「0.3 的计费增长点清单」逐处在增长前计费（按**元素**计，不是按外层 vector 计一次），`assemble`/`instruction_ranges`/`successors`/`plans` 各加 `poll()`；以零/恰好/超限、乘积状态、装配期取消反例证明停止且不发布 `CallContexts`，成功时保留真实 payload（R4、D33/D39，A14）
...
P1 与验证维护均已归档。原任务 1.1–1.3、2.1–2.5、3.1–3.3 共 11 项有交付记录；新增 0.1–0.5 后为 **16 项待办**（0.1、0.2、0.4、0.5 已完成并各自独立复核 Approve、CI 全绿并记录 run；0.3b 待做——0.3 原为单任务，因一次派发在分析阶段停滞无产出而按「pass 表」与「存储计费」拆成两个可独立验证的小片）。3.4 的工作区候选**已实现但未验收**——它阻塞在 0.3（派生存储计费）与 R2/R3（`ret` 值流、异常路径 locals）上，且 3.1 的 CI 记录与其未提交状态不构成背书。当前顺序：**0.3b → 3.4（按新契约重写值流与异常上下文）→ 独立 layer-jarde-crates → 3.5 → 4.x → 5.x**。拆包的任务与回归门槛见 [layer-jarde-crates/tasks.md](../layer-jarde-crates/tasks.md)，不混入 0.x/3.4 修复；已勾选不表示本轮发现的缺陷已消失。
