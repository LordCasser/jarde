...
首版最多 W 个活动类。令 `W = min(requested_workers, floor(max_buffered_result_weight / max_result_weight))`，每个活动类**预留**一份 `max_result_weight` 的交付额度；增加 jobs 不会缩小单项能力，也不会让最早类与后续类争最后一份输出空间。超大结果在形成/交接的检查边界返回带原因的局部停止，不能卡在“永远放不下”的等待中，也不截断 Java 文本后标成功。必要的停止控制事件有固定的小槽，与普通产物额度分离并计入窗口配置；控制槽只携带既有工作序号、停止码和计数/边界，不复制无界正文或诊断消息。

**2026-09-21 修订（窗口的二级额度）**：首版把"每活动类一个槽"当成硬上限，于是已经算出结果的 worker 只因为协调器还没走到它就必须停手——实测 `place_method` 等待合计 8.1 s（bcprov）/25.7 s（s2-009，线程时间），协调器等最早类的 `take_front` 等待约等于整个墙钟（`evidence/cost-attribution.md`）。修订为**预留槽 + 共享池**：

- 每个活动类仍**预留**一份 `max_result_weight`：最早类永远有自己的位置，进展条件与首版相同；
- 预留之外的窗口权重进入**共享池**，任何类的 worker 都可以在其中再放至多若干份结果（由 `max_buffered_result_weight − W × max_result_weight` 决定，至少 0）；池满才等待；
- 交付顺序不变：按物理与声明序，逐方法发布；池只影响"能提前算多少"，不影响"先发谁"；
- 单项容量仍与 worker 数无关：池容量只随声明的总窗口变化，`max_result_weight` 不因 jobs 改变；
- 内存上界不变：活动类预留 + 池 ≤ `max_buffered_result_weight`，仍不是 RSS 上界（与上文口径一致）。

这条修订**放宽**了首版"每活动类最多一个待交付结果"的措辞，但保留它的两条实质承诺：单类不会饿死、总量有界。相应的验收：慢首类 + 快后类下顺序稳定、`buffered_weight_high_water ≤ limit`、池为 0 时行为与首版逐项一致、`jobs` 改变不缩小单项可接受上限。

物理遍历不阻塞在向慢 worker 发送新类上：满窗口时先消费最早槽，空位出现再扩展发现。协调器持有最早类时可立即交付其方法，不必等它整类完成。即使其它 W-1 个槽全满，最早类仍有自己的预留；这给出无容量环路的进展条件。选择更小的窗口可能降低吞吐，但不会增加内存。
