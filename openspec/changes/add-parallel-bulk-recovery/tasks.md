...
- [ ] 4.7 按 design 决策 5 的 2026-09-21 修订实现窗口的二级额度（每活动类预留一份 + 其余进共享池）：池容量只由声明总窗口与 worker 数推出、在报告中可见；保持交付顺序、单项容量与 jobs 无关、总量有界与"最早类不饿死"。验收：慢首类+快后类顺序稳定；`buffered_weight_high_water ≤ limit`；池为 0 时行为与首版逐项一致（反例：把预留也丢进池 → 最早类可被饿死，用例必须变红）；`place_method` 等待与墙钟在 bcprov/s2-009 上的前后对照（3 次中位数、纯构建）。
...
- [ ] 6.3 跑超容量单次 sweep、大类倾斜、慢 sink、取消和普通小单请求回归；记录吞吐、尾部、工作集和同步瓶颈，作显式默认配置裁决，未证实收益不填成功，功能交付与性能主张分别验收（B03–B10）。 已记录的瓶颈：并发从 1→2 有收益、2→4 无进一步收益，编码/写出、总账锁竞争、类倾斜和有序背压均是待测候选，不能由 2→4 不加速唯一归因；见 verification §3。**未做**：超容量 sweep、大类倾斜、慢 sink 与默认配置裁决。
