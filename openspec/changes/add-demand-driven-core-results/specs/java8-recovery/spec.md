...
证据选择只改变可选证据的物化与交付，不改变规则前提、值/控制流/effect 验证、Structured 接受集或拒绝决定。规则验证仍必须完整执行到本次请求允许的范围；完整证据选择 SHALL 保留本 requirement 原有的全部审计语义，任何较精简选择都必须交付核心缺口、拒绝原因、可观察生产者及可按精确产物重建的 origin。关闭详细规则、区域、命名或映射明细 MUST NOT 改变正文、名称、类型、分组、求值顺序、quality/content 或拒绝范围；若详细证据本身在正文提交后停止，必须单独报告证据覆盖与真实 execution。

本 capability 中有关规则审计记录及公开 source map 的交付要求统一遵循所选证据范围：完整选择继续执行原场景的全部交付断言；未选择时保留内部依据、核心拒绝与可核对的产物绑定，不构造完整公开明细。所有模式的值、类型、分组、求值顺序、异常与副作用要求不受选择影响。
...
#### Scenario: Essential and complete evidence use the same rule decision

- **WHEN** 同一不可变方法在足够预算下分别请求默认精简结果与完整规则/区域/映射证据
- **THEN** 两次都执行相同的规则前提验证，正文、名称、类型、分组、求值顺序、Structured/拒绝决定和核心缺口逐字一致；完整选择只增加可定位审计记录

#### Scenario: A minimal result still retains a refusal boundary

- **WHEN** 某区域因未证明值、求值点、转换或 effect 被拒绝，调用方关闭详细规则记录
- **THEN** 结果仍保留拒绝区域、原因、相关可观察生产者和可重建 origin；关闭明细不得把拒绝区域变成 Java、删除字段/调用/类初始化等 effect，或升级 quality/content

#### Scenario: Cross-method evidence remains reconstructible

- **WHEN** accessor、lambda、callee 或静态字段的规则证据被精简交付，正文仍引用其结果
- **THEN** 核心结果保留重建所需的 caller/callee 物理身份、BCI/CP 和派生关系；后续在新预算中展开明细时只能绑定同一确切产物，不能凭同名成员或相同 BCI 改指向

#### Scenario: Detail delivery stops after a committed artifact

- **WHEN** 正文已提交，随后选择的详细规则/区域/映射记录因预算或取消停止
- **THEN** 已提交正文及其语义质量保持不变，详细证据报告实际前缀/未完成和费用，整体 execution 诚实为非 Complete；不得删除正文、补造完整审计或把证据停止解释为恢复拒绝

#### Scenario: Full evidence preserves every original audit scenario

- **WHEN** 调用方选择完整证据模式并给予足够预算
- **THEN** 本 requirement 现有每个规则场景的原有输出、source map、拒绝和受控行为验收全部继续成立；精简模式不得成为绕过任一场景的替代成功路径
