...
- [x] 7.1 跑完整配置确定性与 Essential/All/局部证据足额语义对照，包含既有算术/receiver/boolean/concat/effect/origin/拒绝 fixture；受控 JDK 编译执行中不得以新增拒绝删掉原正确样本，已知失败独立修复后重冻候选（D03/D04/D10）。 证据：`tests/d5_semantic_comparison.rs`——同 (成员, 选择) 两次运行（去 `elapsed_millis`）整份报告逐字段相同（237 对）；三选择（essential / 局部 / all）正文与平面、outcome、fallbacks、每条诊断逐字相同（79 个成员，158 次 ReadDetails 真的发布）；局部 == 完整的"有序子序列 + region 精确相交"投影；受控 JDK 下 essential 与 all 各编一份、**class 文件逐字节相同**，运行值与 fixture 自身一致，且没有用新拒绝删掉原正确样本。
- [x] 7.2 执行 debug/release 子进程、深表达式正常结束与各阶段停止释放、cache clear/容量拒绝、慢消费者及放弃序列门禁；证明无 abort、无遗留工作、真实费用和有界所有权（D06/D11）。 证据：`tests/d5_process_release.rs`（4 子进程 + 1 父进程，debug 与 release 各 5 passed）——8/1024/131072 层深表达式正常结束并产出正文，最深一条在 `ir_items=1000` 停下且 stop 归因 `IrItems`，丢弃后 `work_after_drop=0`；15 个计数维度逐个分类（低于需求即停且归因正确 / 被容忍并记录 / 该 body 不收费）；放弃序列（列举 0 work、换方法各自 1 次物化/解码/呈现、丢弃后 0）；一 entry store `stored=1 refused_capacity=1 hits=1`、`clear()` 后为 0；慢消费者拿满 18 条、`final_delivered=true`、缓冲权重高水位 ≤ 上界。父进程解析子进程自报数字而不是只看退出码。
- [x] 7.3 按父性能协议冻结导航/恢复/证据追问/分页与放弃工作负载，保留相同完整产出和不同证据选择两组对照；记录首次结果、全序列 CPU/墙钟、构造数、返回字节、RSS 与保留权重，时间主张至少十次独立交错样本且无事后挑选（D12）。 判据已事先固定：见 verification §7.3（7 条工作负载、两臂必须同正文、六个度量、≥10 次交错、中位数与极值、结论分三级）。 证据：`evidence/measurement-7-3.md`（两语料 × 7 工作负载 × 2 臂 × 10 次交错，中位数与极值，逐次 RSS），原始样本 `evidence/raw-7-3-{bcprov,s2-009}.jsonl`（各 157 行），harness `examples/demand_workloads.rs` + `evidence/measure_7_3.py`。结论分三层写：契约（两臂返回字节在 5 个工作负载上完全相同、`recover-one` 差值恰为可选记录）、工作量（sweep 峰值 RSS +8.2/+8.8 MiB，其余持平）、时间（不主张臂间加速；首结果 ~2.5 ms vs 整包 1.8 s/6.2 s 作为观测值报告）。口径事故也留在文档里：首次用 `snapshot_all` 量了 WAR 根容器而非整包，且 harness 未挂 store（5.7M entries vs 10,358）。
- [x] 7.4 对每项 D01–D12 保存正例、反例/变异、命令与原始证据，区分“契约通过”“工作量下降”“时间收益未证实”；以关闭明细却仍构造全表、旧匹配序号重扫等变异证明门禁有效（D01–D12）。 证据：`tests/d5_acceptance_ledger.rs` 的模块文档即台账（D01–D12 各行列正例/反例/命令/三类结论），并有一个机械门禁校验 19 个指针都在仓库内、12 行齐全、三种结论都在。两条变异真红后还原：关闭明细却仍构造全表 → 构造计数红；续页重新解码已发布成员 → `code_bytes` 193≠104 且拼接页重复。只有正例的行（D03/D05/D06/D07/D09/D10/D11）逐行注明；D12 交给 7.3。
- [x] 7.5 运行 fmt/clippy/workspace 及受影响 source-map/query/fingerprint 门禁，更新公共示例、能力清单、verification 与架构状态；执行 OpenSpec strict、本地链接与 MODIFIED 场景保留检查，真实实现完成后再同步归档主 specs（D01–D12）。 证据：`cargo fmt --all -- --check` 干净；`cargo clippy --workspace --all-targets --all-features --locked -- -D warnings` 零告警（含新示例与新测试）；`cargo test --workspace --all-targets --all-features --locked` 1578 passed / 0 failed；受影响门禁单列：`p1_xref_golden`、`p1_query_*`、`p5_corpus_fingerprint`、`p5_shared_payload`、`p3_declaration_handoff` 全绿；公共示例 `examples/demand_workloads.rs` 与 `task_operations.rs` 显式声明证据选择；能力清单（`docs/support-matrix.md`）新增三行；OpenSpec strict 24/24。
