...
- [ ] 7.3 按父性能协议冻结导航/恢复/证据追问/分页与放弃工作负载，保留相同完整产出和不同证据选择两组对照；记录首次结果、全序列 CPU/墙钟、构造数、返回字节、RSS 与保留权重，时间主张至少十次独立交错样本且无事后挑选（D12）。 判据已事先固定：见 verification §7.3（7 条工作负载、两臂必须同正文、六个度量、≥10 次交错、中位数与极值、结论分三级）。
