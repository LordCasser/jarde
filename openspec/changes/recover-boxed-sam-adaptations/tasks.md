...
- [ ] 2.3b Builder 仅对已证数组 impl 且零捕获、唯一 SAM 长度参数的站点发 `T[]::new`；捕获长度的 `Supplier<int[]>` 不得误投影，其它装箱 SAM 使用点恢复含箭头调用的语句，来源 BCI 锚点与合成方法物理报告保留。真实 fixture 的方法级文本与类级保守回退均验证，不以单元构造的 ClassMembers 代替集成证明。
