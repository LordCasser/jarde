...
- [ ] 1.1 固定包含当前正确性修复的 baseline revision、构建参数及 fixture 摘要，复核 design 的源码观察；在实施期 verification 中列出仍存在、已修复和未确认项，验证未混入其它 change 的行为差异（A13/A18）。
...
- [ ] 2.1 O1：调查定向访问、操作内目录/backing 贯通和跨请求保留的贡献；复用 `bound-container-lookup` 的计数/fixture，提交 B/D/C/W/F 对照计划和唯一实现归属，验证 sibling/深度/重复名/容量场景齐全（A07/A08/A15/A16）。
