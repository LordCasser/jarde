...
2026-09-21：全量多线程需求已确认；[add-parallel-bulk-recovery](../add-parallel-bulk-recovery/tasks.md) 唯一拥有 O2 恢复类准备、O4 bulk、O7 类间并行及必需的 O5/O8，实现 **0/22**。两份任务表各自计数；完成子项规划不勾选本专项调查，也不等于父专项或实现完成。

## 1. G0 固定基线与测量入口

- [ ] 1.1 冻结 baseline revision、构建参数及 fixture 摘要，复核 design 的源码观察；登记 `85828c4`、`8586356` 的历史行为边界，按当前完成复核及独立归档更新正确性修正、开放反例和 clone/toString 待核实项。正式候选按声明工作负载验收，区分仍存在、已修复和未确认项，验证未混入其它 change 的行为差异（A13/A14/A18）。
- [ ] 1.2 扩展现有 P5 harness 的 W1–W5 固定工作负载与已明确的 W6a 全量 1/2/4/6 worker 序列，另登记 W6b 跨请求并发/合并的宿主需求或缺失理由；验证启动/open/准备/请求/输出与序列总成本可独立复现，不依赖临时 benchmark 目录。
...
- [ ] 2.2 O2：测量 W3/W4 的 class 物化、结构解析、方法定位和 driver/callee 重读；跟踪 bulk 子项的恢复类准备与共享 ownership，查询消费者另行调查，验证分步候选的独立计数、内存代价和歧义/策略约束，不重复实现。
...
- [ ] 2.4 O4：比较 W2/W3/W5 原顺序与 bulk 子项的按类准备/逐方法交付，复核总预算、输出顺序及背压门禁；验证冷端到端包含准备和输出，不把规划中的 recover_all/export 或完整类源码当作既有能力。
