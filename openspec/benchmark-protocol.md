# 重新 benchmark 协议（2026-09-20 复核后）

本文件登记**已完成的轮次**及其参数、判据与资产位置，也登记下一次测量须遵守的同一套规则。当前正式候选是 `ba2076a`（见下），其报告、原始样本、门禁抽样与差异记录已入库到 `evidence/benchmark-ba2076a/`，不再只存在于 `/tmp`。上一次测量（`cd6f2f0`）的历史报告路径为 `/tmp/jarde-bench/jadx-vs-jarde-benchmark.md`，其原始数据已丢失、脚本未完整保留，因此那一轮**不能**被当作可复现证据引用；任何新轮次都必须在声明 SHA 的独立 worktree 里构建并保存上述资产。

## 冻结基线

- **正式候选：`ba2076a`（2026-09-22 固定）**。本候选的引擎与当前 HEAD 的引擎**相同**：`ba2076a` 之后只进过 tests 与文档提交（`b3c65a8`、`235c257`、`9153daf`），`git diff --name-only ba2076a HEAD -- src crates Cargo.toml Cargo.lock` 为空。构建 `--release --locked`，在独立 worktree 与独立 `CARGO_TARGET_DIR` 中完成，`jarde-cli` 二进制 sha256[:16] `7728e9766c88cc19`；语料 12/12 逐字节与下表一致（完整 SHA-256 见 `evidence/benchmark-ba2076a/corpus5.json`）。测量期间机器负载中位 4.63/12，峰值 24.56 出现在 8-worker 格子自身（`load.log`）；page cache 初态未控制，只逐配置预热，因此时间列是**形状证据**，不是稳态收益。
  - **本轮起只有 C/D 臂可比**：实测为 `jarde-cli`/`bulk_scope_sweep` 的 `discard`/`encode`/`write` 三档 sink × `1/2/4/8` worker，每格 10 次交错重复加逐配置预热，240/240 样本 `exit=0` 且交付计数逐项一致（`bulk.jsonl`）。旧 A/B 逐方法形状的数字属于 `8807fa5`，**跨形状不可比，只作历史锚点**。实测（w1 → w8，w8/w1）：bcprov discard 3.44 → 1.86 s（1.85×）、encode 3.78 → 1.94 s（1.95×）、write 4.13 → 2.02 s（2.05×）；s2-009 discard 11.62 → 6.51 s（1.79×）、encode 14.12 → 7.41 s（1.90×）、write 15.89 → 8.50 s（1.87×）。w8 与 w1 的 min–max 不重叠。
  - **本轮未测（不得读作已测）**：① A/B 逐方法形状未重跑（属 `8807fa5`）；② 零容量 store 与超大类倾斜未通过 CLI 跑（CLI 不暴露 store 配置，其功能边界由 `bulk_recovery_retention`/`bulk_recovery_handover` 的计数门禁承担）；③ 导出流的证据选择未作单独对照臂（CLI `export` 固定 `RecoveryEvidenceRequest::all()`，库侧 `with_evidence` 未跑）。另据复核补入：④ **每个语料的完整环境声明本身**必须逐条记录，它是承重件而非背景——bcprov 为 `snapshot_all` + `PlainJar`；s2-009 为 `artifact_tree root` + **53 个显式 container root**（含 `WEB-INF/classes/` 前缀），而上面那条 `resolution_definition_unbound` 正是这份声明的直接后果；⑤ **C/D 臂用的是示例 `bulk_scope_sweep`，不是 CLI**：CLI `export` 是另一条路径（jobs auto / 1 = 2.19 / 9.04 s），两者不得混读；⑥ **`write` 档的落点不同**：示例写固定临时文件，CLI 写调用方指定并以 `create_new` 打开的文件；⑦ **门禁抽样取自导出流**，该流不含 `regions`，所以结构性门禁实际只用了 `quality` + `content`（与上一轮从逐方法 sweep 抽样、带区域门禁的口径不同），复现者需要知道；⑧ 进程级跨工具对照的目标是 `ASN1Object.<init>` 这一个方法，不是随机抽样。
  - **两套 harness 不可互换（已定位的结构差异，不是噪声）**：同一 revision（`ba2076a`）、同一单元格（bcprov / discard / w1 / roomy，两边 store 容量逐位相同 `1<<14, 1<<27`）、同机**交错 10 轮**、两边都用进程外墙钟：G0 进程内 harness 中位 **3.841 s**（3.812–3.888），协议驱动 **3.414 s**（3.391–3.438），配对差 **+0.429 s（1.125×），10/10 同向**（+0.384…+0.484）。两个嫌疑已排除：不是负载漂移（交错），不是仪器开销（`instrument=on/off` 进程墙 3.849 / 3.856 s，差 <0.2%）。差值定位在 G0 自己的账本里：`open 2.9 ms + prepare 266 ms + request 3,509 ms = 3.779 s`（进程墙 3.856 s），而协议驱动的 3.414 s 覆盖**同样的交付**（19,865 条 / 14,495 体，`produced 12045 / explanation_only 2447 / not_produced 3 / no_body 508` 逐项相同）；即 G0 有一个 ~0.26 s 的独立 `prepare` 相是协议驱动路径里没有的，其 `request` 相本身还比协议驱动整个进程墙大 ~95 ms。两套跨会话各自稳定（协议 3.438 → 3.414；G0 3.80 → 3.84–3.86）。**结论：G0 数字用于归因，不与协议行并列比较，也不取平均**；逐轮表与可复现命令见 `evidence/benchmark-ba2076a/harness-diff.md`。
  - **退化形状**：1 MiB 输出额度下 `exit=4`、写入可读前缀（`degenerate.jsonl`）；库侧同形证据见 `add-parallel-bulk-recovery/verification.md` §12。
  - **逐条诊断词汇**：本候选上 `resolution_definition_unbound` 在 s2-009 的 298 个方法（28 类）上出现。准确读法分三段：**位置**——该码在 `delivery.analysis` 平面（`analysis.execution.reason.code`、`analysis.stages[0].state.code`、`analysis.diagnostics[0]`），**这 298 个方法记录 `state=recovered`，是交付了的**，不是被拒的方法；只有 28 个 `class_end` 把它写进自己的 execution 平面，操作级 `partial` 来自那里。**语义**——不是"两个候选、引擎拒绝选"：引擎按 loader 搜索顺序确实选了 ordinal 46，而正在分析的物理定义是 ordinal 17，因此是"该定义不是声明的 loader 会选中的那一个（被同名定义遮蔽），不在它上面建立运行时语义"（引擎原文：`the definition is not bound to the loader that claims it, so no runtime semantics are built on it`）。**触发条件**——本轮 s2-009 声明了 53 个显式 container root，把两份拷贝都纳入；调用方只声明其 loader 会选中的那个容器时该码不出现。MUST NOT 读作恢复失败；消费方仍须按本文件开头的"每个码标识该版本记录了哪个事实"规则解读。
  - **跨工具行**：同轮 jadx 1.5.6 为 bcprov 2.72 s、s2-009 6.02 s（默认 6 线程）、`-j 1` 4.71/10.46 s、峰值 RSS 1.9–3.9 GB，而 jarde 峰值 RSS 78–134 MB。**产出物不同**（jadx 整类源码 2,419 文件；jarde 逐方法记录 bcprov 19,865 条/390.8 MB、s2-009 71,582 条/3,609.6 MB，含四平面与诊断），因此这些数字是**同轮对照**，不构成吞吐胜负结论；进程级单次对照（`recover` 一个方法 11.22 ms/14.2 MB 对 jadx `--single-class` 925 ms/448 MB）同样只是形状差异。原始样本与报告见 `evidence/benchmark-ba2076a/`（`report.md`、`bulk.jsonl`、`cli_export.jsonl`、`jadx.jsonl`、`degenerate.jsonl`、`classification.json`、`sample.json`、`load.log`、交接说明 `handoff.md`）。
- **`85828c4` 与 `8586356` 分别保留为历史比较臂**：前者存在 R1/R2 停止传播缺口，后者已修正。后续修正按各自提交登记；[第四轮复核](completion-review.md) 在包含 `f4d1044`、`2895bc4`、`5c35cbf` 的构建确认 T1–T4 原判据关闭，同时新增 T5（append 的 int 参数消费 char 时丢转换）。T5 已在 `make-required-conversions-explicit`（实现 `c5f8185`）关闭，并在本候选上被独立复核复现验证：产物为 `"" + (int) c + "!"`，贴回编译执行后 `'A'`/`'0'` 逐值与原类相同。新正式候选须固定 SHA、构建配置、目标集合与 T5/CLI 文档预算等边界，不删失败项来获得收益；debug/release 栈深结果分别保留，不能用结构性平面替代正确性验收。
- 正式测量在每个声明 SHA 的独立 worktree 构建，给它独立的 `CARGO_TARGET_DIR`，记录构建参数与二进制摘要。历史臂可用 `git worktree add /tmp/jarde-bench-base 85828c4`；当前候选必须使用实际验收 SHA。两侧行为差异单列，不能归因成性能收益。
- **每份记录同时登记该引擎版本的诊断 code 词汇**：诊断码 SHALL 标识「该引擎版本在本次运行中记录了哪个事实」，MUST NOT 被读作跨版本可比的量。同一个码在不同版本的含义变化（例如声明事实交接前 `jre_declaration_class_not_in_run` 在单个 artifact 上出现 10,720 次、交接后语料全局 0 次，而 `jre_declaration` 出现在每个 artifact 上）MUST 被登记为词汇/事实来源的变化；消费方、比较报告与文档 MUST NOT 把某个码的出现次数下降直接读作质量或覆盖改善，比较 MUST 同时声明两个引擎版本与各自的 code 词汇。
- 每个 change 的实现提交与其 CI 结果：

| 批次 | Change | 实现提交 | CI |
| --- | --- | --- | --- |
| 正确性 | close-recovery-correctness-gaps | `fd0aae8` | 35490735745 success |
| benchmark 1 | expose-recovery-content | `7d095ce` | 35494183220 success |
| benchmark 2 | carry-declaring-class-evidence | `618de49` | 35495275106 success |
| benchmark 3 | bound-container-lookup | `b22ea04` | 35496369587 success |
| benchmark 4 | bind-prefixed-load-roots | `fbcf06b` | 35497694448 success |
| 易用性 1 | add-artifact-navigation | `e0c83b6` | 35499237358 success |
| 易用性 2 | add-task-oriented-operations | `2428752` | 35501294803 success |
| 易用性 3 | add-task-oriented-cli | `85828c4` | 35502305001 success（归档验证记录，本轮未重查远端） |
| 停止传播修正 | preserve-task-operation-stops | `8586356` | 见该 change 的归档验证记录 |

## 语料与每次计时前的门禁

12 个 artifact（vulhub checkout，只读）。**每次计时前校验**：若字节与下表不符，冻结记录必须显式说明语料已变，而不能沿用旧比较。

| artifact | 类型 | 字节 | sha256[:16] 期望 |
| --- | --- | --- | --- |
| nacos/CVE-2021-29442/evil.jar | jar | 2854 | `9217f55601a388e5` |
| weblogic/weak_password/decrypt/weblogic_decrypt.jar | jar | 19309 | `bb5357d7f0dade90` |
| weblogic/weak_password/decrypt/lib/bcprov-jdk15on-152.jar | jar | 2903072 | `5329ddefb3c92927` |
| weblogic/weak_password/web/hello.war | war | 1927 | `b47d85c9b6ec0e2c` |
| struts2/s2-001/S2-001.war | war | 3296352 | `c2e74165ae8b645b` |
| struts2/s2-005/S2-005.war | war | 3272437 | `1c92a38a3618a44b` |
| struts2/s2-007/S2-007.war | war | 3384417 | `9b9b5e37ea721ed9` |
| struts2/s2-008/S2-008.war | war | 3384218 | `fa139b2d545b6ba3` |
| struts2/s2-009/S2-009.war | war | 12272179 | `dda30ca7a2587391` |
| struts2/s2-012/S2-012.war | war | 3383421 | `73d40e42f7866332` |
| struts2/s2-013/S2-013.war | war | 3383341 | `c8f5c7e5c4140392` |
| struts2/s2-015/S2-015.war | war | 3510089 | `23274e700381f13f` |

旧运行的 sha256 只保留前 16 位，只能核对历史摘要前缀一致，不能恢复旧完整摘要。新 campaign 必须计算并保存完整 SHA-256、精确 artifact/方法清单、抽样与排除规则；前缀相同不能代替新样本的完整身份记录。

## 工具

以下是旧运行的工具/启动行为记录；新运行须实际记录路径、版本、JVM 与线程配置，不假定当前环境相同。

- jadx **1.5.6**（`/opt/homebrew/bin/jadx`），JVM OpenJDK 23.0.1。
  - **计时臂**：默认（6 线程）与 `-j 1`。
  - **join 臂**（只为对账类/方法集合，不计时）：`--no-res --output-format json --no-inline-methods --no-inline-anonymous --no-move-inner-classes`。
  - jadx 1.5.6 启动期会拉起 `localhost:8085` 的 MCP server：**计时期间任何时刻只允许一个 jadx 进程**。
- jarde：冻结 worktree，`--release` 构建。

## jarde 测量形状

1. **按需单请求**（进程级）：记录 `jarde-cli` 对单方法/entry 的 wall time，以及 jadx `--single-class` 的类级 wall time。两者交付单位不同，必须分列，不直接宣称同口径加速；要作同口径对照，须另行固定相同类/方法集合与输出工作。
2. **逐 artifact sweep**（旧调用方式的比较臂）：**每个 artifact 一个 snapshot**，**每个方法请求一个新 `Budget`**；容器声明见下，保留抽样规则作为历史比较锚点。
3. **全量批量导出**（2026-09-21 新规划）：以 [add-parallel-bulk-recovery](changes/add-parallel-bulk-recovery/design.md) 为实现和验收依据，一次进程/操作共享总预算，以固定物理范围全部声明为分母，记录无 Body、失败、仅解释及未知后缀。该能力尚未实施，不把以下比较臂写成已经跑出的数据。

新全量 campaign 固定 A–E：A 为旧逐方法无 store，B 为同形状共享 store 且从空开始，C 为 prepared class 的串行 bulk，D 为 C 的 2/4/6 worker，E 为零/超容量、慢 sink 和大类倾斜。先以各路径支持的同一方法集合及行为基线核对语义，再测性能；B 的 warm 请求 p50 不能替代 B 的冷总耗时。C/D 使用同一 JSONL schema 和输出范围，普通单请求默认 cache 策略不随 bulk 改动。

主指标从冷进程开始到全部输出 flush/close；输入打开、发现清单、准备、恢复、编码和写出均计入。每配置至少 10 次独立交错重复，同时保存 CPU/RSS、首次结果、完整产出分类和原始计数；OS page cache 与应用 cache 初态分开声明。jadx 的 1/N 线程、版本/JVM、resources/nested 范围和构造器/内联差异必须记录；整类源码与方法产物流分列产品场景，不能直接当作等价交付。

旧数据中 40.2 s / 14,495 约为 2.773 ms/体，296.5 s / 53,516 约为 5.540 ms/体，均不同于请求 p50 的 2.65/5.10 ms。0.19/0.10 ms 的 store-on p50 只支持请求延迟观察，不证明全量总时间已经追平 jadx；`analysis_steps` 相同也不能证明分析耗时相同。正式数值由完整 campaign 填写。

## 环境声明是承重件（必须记录并哈希）

前缀 load root 现在由调用方**显式声明**，引擎不推断。因此每个 artifact 的完整声明必须逐项记录并哈希，否则 `archive_entries`/`read_bytes` 的下降无法归因到定向访问——可能只是声明变了：

- 每个 loader 的 roots，逐条：`container` origin（snapshot + container chain）与 raw `prefix`（普通 JAR 用空 prefix；WAR 的 `WEB-INF/classes/` 必须显式写）；
- `RuntimeProfile`：`java_release = 8`、`MultiReleasePolicy::Disabled`、`LayoutMode::Generic`；
- `LoadDomain`：loader 名、`parent_loader`、delegation、roots 顺序、module mode；
- 是否挂 `FactsCache`（默认 off；如启用，记录容量配置与预热步骤）。

## 记录列（固定）

**每请求分布**（p50/p90/max，逐 artifact）：`archive_entries`、`entry_bytes`、`read_bytes`、`class_bytes`、`class_headers`、`method_bodies`、`analysis_steps`、`result_items`，以及 wall µs。

**新增列**（本次首次启用，旧数字不在同一口径）：

- `RecoveryReport.content`：`not_produced` / `explanation_only` / `contains_statements`——**主分类器**，结构性事实；
- 容器计数：目录解析次数、nested 物化次数与字节、retained weight 与容量拒绝（cache 启用时）；
- 旧的 token/调用序列/字符串相似度比率**保留**供与旧数字对照，但必须标注「分类列口径已变：旧数字来自去注释 token 启发式，新数字来自引擎的结构分类」，不得把两者当同一口径。

**结果平面**：representation / quality / syntax / compile / semantic / verification / coverage / execution 各自独立记录；拒绝原因按码计数、按 artifact 分列。

**计数与比较行的声明**：每个计数字段 SHALL 被读作「本次请求在其声明范围与请求形状下实际做了多少工作」的代理，MUST NOT 被当作跨请求形状可比的单位。比较行 MUST 声明请求形状、声明的 roots/profile（含 multi-release/layout 策略）与引擎版本，并明确该 scope 下这个数「数的是什么」；MUST NOT 跨形状直接比较计数，也 MUST NOT 用未声明形状的数字得出收益或回归结论。

## 重复与负载

- 正式 G0/G1 对照：每工具/配置至少 **10 个独立重复**，交错执行并重建声明初态，保存全部有效原始样本和负载；进程内重复 **6 次**仅作 warm 观察，不能充当独立样本。`--version` 启动参照 **5 次**单列。尾延迟结论另定足额样本。
- 实验前登记主指标、退化/内存界限与统计方法，发布中位数及区间，不事后挑样本/改阈值。旧记录的“非空载只认 >2×”只是粗略观察规则，不构成显著性或性能验收标准；无法区分噪声时如实标记未证实。
- `archive_entries`/`read_bytes` 是实际工作的计费代理；retained weight 是驻留代理，**不是** RSS（RSS 要单独测量，不与计费混谈）。

## join 与对账

- **物理清单保留全部定义。** 身份沿用 snapshot + 完整 container/entry 链（各层 ordinal 与 raw name）+ class 字节身份/variant；方法再带 member ordinal、raw name 和 descriptor。同名、同签名、甚至同字节而不同 origin 的记录不合并。ordinal 用于保存 multiplicity，缺少它的一侧只能在候选唯一时完成映射，不能任取第一项。
- **跨工具先映射来源。** jadx 的 `dex`/输入来源记录映射到同一输入清单中的容器和 class 条目，再比较方法声明。保留完整类名；禁止只取 basename、从 entry 路径直接猜 `this_class`，或用集合去重抹掉多 origin。jadx 重命名/内部类展示须由原始符号映射处理，不能把源代码展示名当作 JVM 物理身份。
- **具备两个分列口径。** 全物理 scope 统计全部定义/声明与各自产物；特定 loader/roots/profile 下的有效类视图按既有解析结果选择。后者是带上下文的视图，不改变物理清单。未指定或不足以确定的环境保留候选/未决，不自选容器优先级。
- **差异逐条归类。** 区分一一匹配、同名另一物理版本、已证实的同字节重复、已证实的折叠/省略、真实失败与来源未决。构造器及 synthetic accessor 的差异也须先对照原始声明和版本；不能仅因名字为 `<init>`/`access$...` 自动排除。各类差异保存两侧原始身份、依据和计数，未决项不能计作已证实的物理盈余或遗漏。
- **来源不全时降级结论。** 只有完整类名 + name/descriptor 时，可以报告名称签名集合差；不能报告物理一一覆盖。`dex=[]` 或多个来源无法辨认时必须标为来源未决。这个口径可以支持版本差异调查，不能通过 B01/B07/B10 的同物理工作集对账。
- **性能与质量分开对齐。** 全范围运行的时间对应其全部实际工作，保留方法/字节分母及范围差异；输出后删除重复行不能把该总时间变成共同集合耗时。严格同工作集性能需预先固定双方可选的同一组物理输入/定义并重跑；工具无法选择时报告产品场景差异。质量可在已一一映射的共同集合上比较，但未匹配项另列，不从任一工具完整覆盖率分母中静默消失。
- jadx 不是正确性证明；一致性指标也不是。行为等价只用受控、可编译的 fixture 做编译/执行对照。

S2-009 的实际区分见 [bulk verification 的来源复核](changes/add-parallel-bulk-recovery/verification.md#11-s2-009-同名多-origin-复核)：57,180 条物理方法记录、56,892 个完整类名签名键、历史 jadx 56,882 个键；288 条重复签名物理记录和 10 个另一版本独有签名不能混称为“物理只多 10 个”。

## 预期变化（用于核对，不作为结论）

冻结基线与 `cd6f2f0` 之间的四项引擎变化，会在同一语料上表现为：

1. 定向 container 访问：单请求的 `archive_entries` / `read_bytes` 应显著低于「一次全树」的水平；
2. 显式 prefix root：`WEB-INF/classes/...` 的 `resolution_definition_unbound` 应在声明了 prefix 之后可绑定（**未声明时仍 unbound**，这正是对照）；
3. 声明类事实同源交接：`jre_declaration_class_not_in_run` 应大幅减少，但 body 质量不因此自动提升；
4. `content` 分类：替代 token 启发式给出结构性分类。

任何一项与预期不符都要如实记录，并把归因所需的声明/配置一并写出。
