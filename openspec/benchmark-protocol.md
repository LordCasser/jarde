...
- **正式候选：`ba2076a`（2026-09-22 固定）**。本候选的引擎与当前 HEAD 的引擎**相同**：`ba2076a` 之后只进过 tests 与文档提交（`b3c65a8`、`235c257`、`9153daf`），`git diff --name-only ba2076a HEAD -- src crates Cargo.toml Cargo.lock` 为空。构建 `--release --locked`，在独立 worktree 与独立 `CARGO_TARGET_DIR` 中完成，`jarde-cli` 二进制 sha256[:16] `7728e9766c88cc19`；语料 12/12 逐字节与下表一致（完整 SHA-256 见 `evidence/benchmark-ba2076a/corpus5.json`）。测量期间机器负载中位 4.63/12，峰值 24.56 出现在 8-worker 格子自身（`load.log`）；page cache 初态未控制，只逐配置预热，因此时间列是**形状证据**，不是稳态收益。
  ...
  - **本轮未测（不得读作已测）**：A/B 逐方法形状未重跑；零容量 store 与超大类倾斜未通过 CLI 跑（CLI 不暴露 store 配置，其功能边界由 `bulk_recovery_retention`/`bulk_recovery_handover` 的计数门禁承担）；导出流的证据选择未作单独对照臂（CLI `export` 固定 `RecoveryEvidenceRequest::all()`，库侧 `with_evidence` 未跑）。
  - **与在库内归因基线的差异（未解释完，如实登记）**：`optimize-demand-workloads` 的 G0 基线用进程内 harness 测同一台机器上 bcprov 的 request 段（挂 store、稳态中位：discard 3.80 s / encode 4.07 s / write 4.47 s），比本轮协议驱动的三档各快 7–8%（3.44/3.78/4.13 s）。两者 harness、构建目录与负载记录都不同，本文件**不取平均、不择优**：协议数字用协议驱动，G0 数字用于归因；该差异保留为待消解的开放项，消解方式是同一 revision、同一 `CARGO_TARGET_DIR` 下用同一驱动重跑一档（尚未做）。
  - **退化形状**：1 MiB 输出额度下 `exit=4`、写入可读前缀（`degenerate.jsonl`）；库侧同形证据见 `add-parallel-bulk-recovery/verification.md` §12。
  - **逐条诊断词汇**：本候选上 `resolution_definition_unbound` 在 s2-009 的 298 个方法（28 类）上出现，原因是这些类在两个容器里各有一份（`commons-logging-1.1.1.jar` 与 `commons-logging-api-1.1.jar`；`ArrayUtils` 同时在 beanutils 与 commons-collections）——是设计上的歧义拒绝，MUST NOT 读作恢复失败；消费方仍须按本文件开头的"每个码标识该版本记录了哪个事实"规则解读。
