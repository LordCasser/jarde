...
当前 P0 的 Foundation 2.1–2.5、容器回归 3.1、classfile 对抗/性质回归 3.2 及预算/取消/局部性门禁 3.3 已由对应任务和测试证明。公共 Engine 与有界 JSON CLI 已通过真实二进制入口验证；bytecode 边界另由受控 JDK 25 oracle 交叉检查，并以固定 ECJ 4.6.1 生成的 45.3–52.0 历史目标语料验证版本读取和 `jsr`/`ret` 边界。CLASS/JAR/WAR、重复项、小型真实 ZIP64、压缩、CRC、快照、MUTF-8、未知/嵌套 attribute、switch/wide、截断和属性/指令性质矩阵已复核；十个预算维度均有边界证据，公共入口预取消及枚举/读取/CP/handler/instruction 中途取消不伪造 Complete，未请求方法的非法 Body 不影响所选方法，P0 生产模块和依赖中不存在 XRef/CFG/SSA/AST/IR。P0 仍未完成 3.4–3.5 的支持矩阵、CI/示例、最终验证与归档出口门槛。P1–P5 及未勾选任务仍为 **planned / not implemented**。只有真实测试记录满足对应任务和 change 的出口门槛后才勾选实施任务并归档。
