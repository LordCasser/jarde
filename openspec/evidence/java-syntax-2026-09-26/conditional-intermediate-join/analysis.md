...
The method's bytecode gives the distinction directly (`original-javap.txt`): outer test at BCI 0 branches to BCI 23; inner test at BCI 4 chooses calls at BCIs 9 and 15; both paths reach BCI 18, where the inner stack value is consumed by `iadd` at BCI 19; the result transfers at BCI 20 to outer join BCI 26. The outer false arm calls `f3` at BCI 23 and also reaches BCI 26, where `ireturn` consumes the final stack value. Thus the inner Phi and outer Phi are distinct: the first has an arithmetic consumer between joins, while the second is the method's return value. The Jarde report additionally records BCI 18 as an uncovered live block. Its generic wording about edges omitted by the normal-flow view does not identify the actual cause in this case; the later root trace below does.

## Root replay: first structural cause

The [region trace](region-trace.txt) was taken with a rebuilt CLI at `ff5847e0`. `JRE_JOIN_PROBE` reports the correct outer post-dominator BCI 26 and inner post-dominator BCI 18. `JRE_PREFIX_PROBE` then reports visited blocks `[0, 4, 23, 9, 15, 26]`, omitting the live BCI 18. The normal-flow view retains the normal `9→18` and `15→18` transfers; the issue is not an excluded CFG edge. `region_at_inner` returns the inner `If` and `next=18`, while the outer `If` consumes the resulting arm run without continuing that arm at BCI 18 under its BCI 26 boundary. Only after closing this structural ownership gap can a conditional-value proof compose the inner Phi, BCI 19 arithmetic and the outer Phi atomically. This is a more precise first cause than the initial report text.
