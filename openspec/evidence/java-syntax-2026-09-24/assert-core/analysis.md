...
实现归属：当前**编译失败**是已有 `recover-conditional-values` change 中的双臂栈 Phi→`putstatic Z` 问题；现有 `Region::If`、SSA、AST/field 消费者足以处理，不必为断言开新解析机制。本证据把正例缩到仅此失败，并为该 change 补了真实状态来源负例。若以后追求源级 `assert` 语法及重新生成 `ACC_SYNTHETIC`，需另行证明合成字段、唯一 `<clinit>` 写入、所有读取、当前类 status 来源及条件/detail 求值门控的整组关系；该跨成员投影不属于条件值 change，也不能由它的成功自动推出。

冻结文件包括两份 class、原源码、JADX/Jarde 完整主体源码、两份 `javap`、补丁与 `replay.py`。最新基线重放摘要由脚本写入调用者指定的输出目录；其两个 Jarde 类均 `javac_exit=1`，两个 JADX 类均 `javac_exit=0`，原/JADX 的启停输出和补丁/JADX 的选择性输出如上。
