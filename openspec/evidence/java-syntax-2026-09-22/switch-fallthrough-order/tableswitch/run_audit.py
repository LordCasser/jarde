...
WORK = Path("/tmp/jarde-tableswitch-fallthrough-audit")
CLI = Path("/tmp/jarde-cli-deferred-budget-908c")
CLASS = "TableswitchFallthrough"
RUNNER = "TableswitchFallthroughRunner"
...
def compile_and_run(source: Path, target: str, result: dict[str, object]) -> None:
    ...
    if result[f"{target}_javac"] == 0:
        package_name = package(copy.read_text()).strip().removeprefix("package ").removesuffix(";")
...
def main() -> None:
    ...
    WORK.mkdir(parents=True, exist_ok=True)
    if not CLI.is_file():
        raise SystemExit(f"frozen CLI is missing: {CLI}")
    actual_cli_sha = hashlib.sha256(CLI.read_bytes()).hexdigest()
    if actual_cli_sha != EXPECTED_CLI_SHA256:
        raise SystemExit(f"frozen CLI changed: {actual_cli_sha}")

    original_dir = WORK / "original-class"
    ...
    if "tableswitch   { // 1 to 4" not in javap:
        (OUT / "summary.json").write_text(json.dumps(summary, indent=2) + "\n")
        raise SystemExit("javac did not produce the expected dense tableswitch; see javap.txt")
    summary["tableswitch_confirmed"] = True
    ...
    if jadx_source is not None:
        ...
        summary["jadx_run"] = None
        (OUT / "jadx.java.txt").write_text("JADX source was not produced; see jadx.log\n")
        (OUT / "jadx-run.txt").write_text("")
        (OUT / "jadx-diff.txt").write_text("runtime comparison skipped: JADX source was not produced\n")
    ...
    if generated.returncode == 0:
        ...
        if summary["jarde_run"] == 0:
            compare(OUT / "original-run.txt", OUT / "jarde-run.txt", OUT / "jarde-diff.txt")
        else:
            (OUT / "jarde-diff.txt").write_text(
                "runtime comparison skipped: recovered source did not compile or run\n"
            )
    else:
        summary["jarde_javac"] = None
        summary["jarde_run"] = None
        (OUT / "jarde-diff.txt").write_text("runtime comparison skipped: class-source failed\n")
