...
WORK = Path("/tmp/jarde-switch-fallthrough-order-audit")
CLI = Path("/tmp/jarde-cli-deferred-budget-908c")
CLASS = "SwitchFallthroughOrder"
RUNNER = "SwitchFallthroughOrderRunner"
...
def main() -> None:
    ...
    if generated.returncode == 0:
        ...
        if summary["jarde_run"] == 0:
            ...
            (OUT / "jarde-diff.txt").write_text("runtime comparison skipped: recovered source did not compile\n")
    (OUT / "summary.json").write_text(json.dumps(summary, indent=2) + "\n")
