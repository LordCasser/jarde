...
def main() -> None:
    ...
    assert jarde_javac == 0 and jarde_runtime == 1
    assert "getDefaultValue()\" is null" in (HERE / "jarde-run.txt").read_text()
    assert "default " not in (HERE / "jarde-FloatDefaults.java").read_text()
