...
OUT = ROOT / "openspec/evidence/java-syntax-2026-09-22/shifts/root-core"
WORK = Path("/tmp/jarde-shifts-root-core-0549")
...
SOURCES = {
    "ShiftCore": r'''public class ShiftCore {
...
def audit(class_name: str, prefix: str, source_names: list[str], runner_name: str) -> dict[str, object]:
    ...
    summary["cli_sha256_after"] = hashlib.sha256(CLI.read_bytes()).hexdigest()
    assert summary["cli_sha256"] == summary["cli_sha256_after"], "CLI changed during audit"
...
def main() -> None:
    write_sources()
    summaries = [
        audit("ShiftCore", "basic", ["ShiftCore", "ShiftEffects", "ShiftBasicRunner"], "ShiftBasicRunner"),
    ]
