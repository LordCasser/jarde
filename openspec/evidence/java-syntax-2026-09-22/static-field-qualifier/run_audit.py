"""Three-way whole-class replay for an expression used to qualify a static field."""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='jarde-static-qualifier-audit-') as raw:
        work = Path(raw)
        classes = work / 'original'
        compiled = run(['javac', '--release', '8', '-g:none', '-d', str(classes), str(HERE / 'StaticQualifierProbe.java'), str(HERE / 'StaticQualifierRunner.java')], 'original-javac')
        assert compiled.returncode == 0
        class_file = classes / 'StaticQualifierProbe.class'
        original = run(['java', '-Xverify:all', '-cp', str(classes), 'StaticQualifierRunner'], 'original-runtime')
        assert original.returncode == 0
        javap = run(['javap', '-c', '-p', str(class_file)], 'original-javap')
        assert javap.returncode == 0
        generated = run([str(CLI), 'class-source', '--input', str(class_file), '--class', 'StaticQualifierProbe', '--policy', 'single-class', '--release', '8', '--format', 'text'], 'jarde-cli')
        ...
        if jadx.returncode == 0:
            jadx_input = next((work / 'jadx').rglob('StaticQualifierProbe.java'))
