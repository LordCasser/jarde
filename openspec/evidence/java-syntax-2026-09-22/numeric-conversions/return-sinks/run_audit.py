...
W=Path(tempfile.mkdtemp(prefix='jarde-return-sinks-'))
O=R/'openspec/evidence/java-syntax-2026-09-22/numeric-conversions/return-sinks'
O.mkdir(exist_ok=True)
S={
'ReturnSinks': '''public class ReturnSinks {
 public int value;
...
'ReturnSinksRunner': '''public class ReturnSinksRunner {
...
def run(args,n):
 ...
 return p.returncode
assert run(['javac','--release','8','-g:none','-d',str(W/'original'),str(O/'ReturnSinks.java')],'source-javac.log')==0
k=W/'original/ReturnSinks.class'
b=k.read_bytes(); patches=[]
for old,new in [(b'()I',b'()B'),(b'(Ljava/lang/Object;I)I',b'(Ljava/lang/Object;I)B'),(b'(ZII)I',b'(ZII)B')]:
...
assert run(['javac','--release','8','-g:none','-cp',str(W/'original'),'-d',str(W/'original'),str(O/'ReturnSinksRunner.java')],'runner-javac.log')==0
assert run(['javap','-v','-c','-p',str(k)],'javap.txt')==0
assert run(['java','-Xverify:all','-cp',str(W/'original'),'ReturnSinksRunner'],'original.txt')==0
assert len((O/'original.txt').read_text().splitlines())==31
cli=R/'target/debug/jarde-cli';sha=hashlib.sha256(cli.read_bytes()).hexdigest()
p=subprocess.run([str(cli),'class-source','--input',str(k),'--class','ReturnSinks','--policy','single-class','--release','8','--format','text'],capture_output=True,text=True,timeout=45)
assert p.returncode==0
(O/'jarde.java.txt').write_text(p.stdout);(O/'jarde-report.txt').write_text(p.stderr)
j=W/'jarde';j.mkdir();(j/'ReturnSinks.java').write_text(p.stdout)
s={'bytes':len(b),'class_sha256':hashlib.sha256(b).hexdigest(),'cli_sha256':sha,'jarde_quotes':p.stdout.count('@bytecode'),'cases':31}
s['jarde_javac']=run(['javac','--release','8','-d',str(j/'classes'),str(j/'ReturnSinks.java'),str(O/'ReturnSinksRunner.java')],'jarde-javac.log')
if s['jarde_javac']==0:
 s['jarde_java']=run(['java','-Xverify:all','-cp',str(j/'classes'),'ReturnSinksRunner'],'jarde.txt')
 s['jarde_equal']=(O/'jarde.txt').read_bytes()==(O/'original.txt').read_bytes()
assert run(['jadx','--no-res','-d',str(W/'jadx'),str(k)],'jadx.log')==0
g=next((W/'jadx').rglob('ReturnSinks.java'));t=g.read_text();(O/'jadx.java.txt').write_text(t)
package=next((v for v in t.splitlines()if v.startswith('package ')),'')
sup=W/'support';sup.mkdir();(sup/'ReturnSinksRunner.java').write_text(package+'\n'+S['ReturnSinksRunner']+'\n')
s['jadx_javac']=run(['javac','--release','8','-d',str(W/'jadx-classes'),str(g),str(sup/'ReturnSinksRunner.java')],'jadx-javac.log')
if s['jadx_javac']==0:
 prefix=package[8:].rstrip(';')+'.'if package else ''
 s['jadx_java']=run(['java','-Xverify:all','-cp',str(W/'jadx-classes'),prefix+'ReturnSinksRunner'],'jadx.txt')
