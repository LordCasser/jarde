...
A=E/'deferred-evaluation/root-final-ecab'
A.mkdir(exist_ok=True)
W=Path(tempfile.mkdtemp(prefix='jarde-root-deferred-accept-'))
cli=Path('/tmp/jarde-cli-deferred-final-ecab');before=hashlib.sha256(cli.read_bytes()).hexdigest()
...
def replay(row):
 ...
 code=code.replace(old,str(out.relative_to(R))).replace('target/debug/jarde-cli','/tmp/jarde-cli-deferred-final-ecab')
