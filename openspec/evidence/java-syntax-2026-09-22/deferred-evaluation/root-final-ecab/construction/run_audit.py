...
R=Path('/Users/lordcasser/workspace/projects/jarde');W=Path('/var/folders/gm/lzn3ylzx4lz7mx29lftngdz40000gn/T/jarde-root-deferred-accept-xvk0cuwy/construction');W.mkdir(exist_ok=True)
O=R/'openspec/evidence/java-syntax-2026-09-22/deferred-evaluation/root-final-ecab/construction';O.mkdir(exist_ok=True)
sources={
...
p=subprocess.run([str(R/'/tmp/jarde-cli-deferred-final-ecab'),'class-source','--input',str(k),'--class','ConstructionOrder','--policy','single-class','--release','8','--format','text'],capture_output=True,text=True,timeout=30);(O/'jarde.java.txt').write_text(p.stdout);(O/'jarde-report.txt').write_text(p.stderr)
