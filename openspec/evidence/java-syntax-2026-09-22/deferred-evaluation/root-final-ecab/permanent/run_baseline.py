...
def audit(name,fixture_dir,runner,support,work,evidence,patched):
 ...
 F=R/'tests/fixtures'/fixture_dir
 def run(args,n):
 ...
 cli=R/'/tmp/jarde-cli-deferred-final-ecab';before=hashlib.sha256(cli.read_bytes()).hexdigest()
 ...
 print(json.dumps({k:v for k,v in s.items()if k!='jarde_differences'},indent=2));print('jarde_differences',len(s.get('jarde_differences',[])))
W=Path('/var/folders/gm/lzn3ylzx4lz7mx29lftngdz40000gn/T/jarde-root-final-ecab-cfmq33vh/permanent')
P=R/'openspec/evidence/java-syntax-2026-09-22/deferred-evaluation/fixture/patch.py'
p=subprocess.run(['python3',str(P),'--work',str(W)],capture_output=True,text=True,timeout=90)
assert p.returncode==0,p.stderr
audit('DeferredValueOrder','p3-deferred-value-order','DeferredValueOrderRunner',['DeferredEffects','OrderValue'],str(W),'deferred-evaluation/root-final-ecab/permanent',True)
shutil.copy2(W/'patch-manifest.json',E/'deferred-evaluation/root-final-ecab/permanent/patch-manifest.json')
