...
EVIDENCE = ROOT / "openspec/evidence/java-syntax-2026-09-22/deferred-evaluation/negative-review/failed-declaration/root-ecab"
WORK = Path(tempfile.mkdtemp(prefix="jarde-root-failed-declaration-")) / "work"
...
def main() -> None:
    ...
    assert summary["patched_sha256"] == "cc8901164d3aac1206b65fc03c69d8798359b88c0bd300c5202ef614ccc79d0b"
    assert summary["original_lines"] == 6 and summary["jadx_equal"]
    assert summary["jarde_javac_returncode"] != 0
    assert not summary["run_body_take_count"] and not summary["run_body_saved_name"] and not summary["run_body_local_name"]
    origins = {v for row in run_source_map_bcis for v in [row["primary"], *row["derived"]]}
    assert {2, 3, 4, 5, 8, 11} <= origins
