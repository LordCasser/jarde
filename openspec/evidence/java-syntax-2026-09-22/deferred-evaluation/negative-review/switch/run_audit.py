...
EVIDENCE = ROOT / "openspec/evidence/java-syntax-2026-09-22/deferred-evaluation/negative-review/switch"
WORK = Path("/tmp/jarde-deferred-negative-switch")
...
def main() -> None:
    ...
    original.mkdir()
    sources = [EVIDENCE / name for name in ("SwitchDeferred.java", "SwitchSupport.java", "SwitchRunner.java")]
    ...
    class_path = original / "SwitchDeferred.class"
    ...
    jarde = subprocess.run(
        [str(CLI), "class-source", "--input", str(class_path), "--class", "SwitchDeferred", "--policy", "single-class", "--release", "8", "--format", "text"],
        capture_output=True,
        text=True,
        timeout=30,
    )
    ...
    jarde_dir.mkdir()
    jarde_source = jarde_dir / "SwitchDeferred.java"
    ...
    jadx_compile = None
    if jadx_rc == 0:
        generated = next(jadx_root.rglob("SwitchDeferred.java"))
