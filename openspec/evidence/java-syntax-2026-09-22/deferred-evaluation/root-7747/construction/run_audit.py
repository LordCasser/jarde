...
R=Path('/Users/lordcasser/workspace/projects/jarde');W=Path('/tmp/jarde-deferred-root-7747/construction');W.mkdir(exist_ok=True)
O=Path('/Users/lordcasser/workspace/projects/jarde/openspec/evidence/java-syntax-2026-09-22/deferred-evaluation/root-7747/construction');O.mkdir(exist_ok=True)
sources={
...
p=subprocess.run([str(R/'/tmp/jarde-cli-deferred-accepted-7747'),'class-source','--input',str(k),'--class','ConstructionOrder','--policy','single-class','--release','8','--format','text'],capture_output=True,text=True,timeout=30);(O/'jarde.java.txt').write_text(p.stdout);(O/'jarde-report.txt').write_text(p.stderr)
