...
k=W/'original/GuardInlineControls.class'
...
p=subprocess.run([str(cli),'class-source','--input',str(k),'--class','GuardInlineControls','--policy','single-class','--release','8','--format','text'],capture_output=True,text=True,timeout=45);assert p.returncode==0
(O/'jarde.java.txt').write_text(p.stdout);(O/'jarde-report.txt').write_text(p.stderr)
j=W/'jarde';j.mkdir();(j/'GuardInlineControls.java').write_text(p.stdout)
...
s['jarde_javac']=run(['javac','--release','8','-d',str(j/'classes'),str(j/'GuardInlineControls.java')]+files[1:],'jarde-javac.log')
if s['jarde_javac']==0:
...
g=next((W/'jadx').rglob('GuardInlineControls.java'));t=g.read_text();(O/'jadx.java.txt').write_text(t)
