...
R=Path('/Users/lordcasser/workspace/projects/jarde');W=Path('/tmp/jarde-deferred-root-908c/composite');W.mkdir(exist_ok=True)
O=Path('/Users/lordcasser/workspace/projects/jarde/openspec/evidence/java-syntax-2026-09-22/deferred-evaluation/composite-boundary/root-908c');O.mkdir(exist_ok=True)
...
cli_hash=hashlib.sha256((R/'/tmp/jarde-cli-deferred-budget-908c').read_bytes()).hexdigest()
p=subprocess.run([str(R/'/tmp/jarde-cli-deferred-budget-908c'),'class-source','--input',str(k),'--class','DeferredComposite','--policy','single-class','--release','8','--format','text'],capture_output=True,text=True,timeout=30);(O/'jarde.java.txt').write_text(p.stdout);(O/'jarde-report.txt').write_text(p.stderr)
...
assert cli_hash==hashlib.sha256((R/'/tmp/jarde-cli-deferred-budget-908c').read_bytes()).hexdigest();summary['cli_sha256']=cli_hash
