...
R=Path('/Users/lordcasser/workspace/projects/jarde');W=Path('/var/folders/gm/lzn3ylzx4lz7mx29lftngdz40000gn/T/jarde-root-final-ecab-cfmq33vh/composite');W.mkdir(exist_ok=True)
O=R/'openspec/evidence/java-syntax-2026-09-22/deferred-evaluation/composite-boundary/root-final-ecab';O.mkdir(exist_ok=True)
sources={'DeferredComposite': 'public class DeferredComposite {public static int callAdd(){return DeferredSupport.value()+1;}public static int callNeg(){return -DeferredSupport.value();}public static int fieldAdd(){return DeferredSupport.field+1;}public static int divAdd(int a,int b){return a/b+1;}public static void keepPool(){DeferredSupport.mark();}}', 'DeferredSupport': 'public class DeferredSupport {public static int trace;public static int mode;public static int field;public static int[] shared;public static final RuntimeException FAILURE=new IllegalStateException("chosen");public static int value(){trace=trace*10+1;if(mode==1)throw FAILURE;return field;}public static void mark(){trace=trace*10+2;field=9;shared[0]=8;if(mode==2)throw FAILURE;}}', 'DeferredRunner': 'public class DeferredRunner {interface Task{Object run();}static void run(String name,int mode,Task t){DeferredSupport.trace=0;DeferredSupport.mode=mode;DeferredSupport.field=5;DeferredSupport.shared=new int[]{7};try{System.out.println(name+mode+":"+t.run()+":"+DeferredSupport.trace);}catch(RuntimeException e){System.out.println(name+mode+":"+e.getClass().getName()+":"+(e==DeferredSupport.FAILURE)+":"+DeferredSupport.trace);}}public static void main(String[]a){for(int m=0;m<3;m++){run("callAdd",m,()->DeferredComposite.callAdd());run("callNeg",m,()->DeferredComposite.callNeg());run("fieldAdd",m,()->DeferredComposite.fieldAdd());run("div",m,()->DeferredComposite.divAdd(10,2));run("divZero",m,()->DeferredComposite.divAdd(10,0));}}}'}
def run(args,path):
...
value=cp(r'#(\d+) = Methodref.*// DeferredSupport\.value:\(\)I');field=cp(r'#(\d+) = Fieldref.*// DeferredSupport\.field:I');mark=cp(r'#(\d+) = Methodref.*// DeferredSupport\.mark:\(\)V')
methods=[('callAdd',b'\xb8'+struct.pack('>H',value)+bytes.fromhex('0460ac'),2,0),('callNeg',b'\xb8'+struct.pack('>H',value)+bytes.fromhex('74ac'),1,0),('fieldAdd',b'\xb2'+struct.pack('>H',field)+bytes.fromhex('0460ac'),2,0),('divAdd',bytes.fromhex('1a1b6c0460ac'),2,2)]
...
cli_hash=hashlib.sha256((R/'/tmp/jarde-cli-deferred-final-ecab').read_bytes()).hexdigest()
p=subprocess.run([str(R/'/tmp/jarde-cli-deferred-final-ecab'),'class-source','--input',str(k),'--class','DeferredComposite','--policy','single-class','--release','8','--format','text'],capture_output=True,text=True,timeout=30);(O/'jarde.java.txt').write_text(p.stdout);(O/'jarde-report.txt').write_text(p.stderr)
r=W/'jarde';r.mkdir(exist_ok=True);(r/'DeferredComposite.java').write_text(p.stdout)
summary={'class_sha256':hashlib.sha256(a).hexdigest(),'patches':patches,'cases':15,'jarde_quotes':p.stdout.count('@bytecode')}
...
if summary['jarde_javac']==0:
 ...
 assert len((O/'original.txt').read_text().splitlines()) == len((O/'jarde.txt').read_text().splitlines()) == 15
...
assert cli_hash==hashlib.sha256((R/'/tmp/jarde-cli-deferred-final-ecab').read_bytes()).hexdigest();summary['cli_sha256']=cli_hash
