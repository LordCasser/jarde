#!/usr/bin/env python3
"""Replay the user-static enum boundary audit from this directory."""
...
def main() -> None:
    ...
    assert run(['javac', '--release', '8', '-g:none', '-d', str(original),
                str(HERE / 'Measure.java'), str(HERE / 'MeasureRunner.java')], HERE / 'source-javac.log') == 0
    assert run(['java', '-Xverify:all', '-cp', str(original), 'MeasureRunner'], HERE / 'original-run.txt') == 0
    ...
    assert run(['java', '-Xverify:all', '-cp', str(jadx_classes),
                'defpackage.MeasureRunner'], HERE / 'jadx-run.txt') == 0
    ...
    sources = sorted(HERE.glob('jarde-*.java'))
    jarde_classes = HERE / 'classes-jarde'
    jarde_classes.mkdir()
    run(['javac', '--release', '8', '-g:none', '-d', str(jarde_classes),
         *(str(path) for path in sources)], HERE / 'jarde-javac.log')
    run(['java', '-Xverify:all', '-cp', str(jarde_classes), 'MeasureRunner'], HERE / 'jarde-run.txt')
