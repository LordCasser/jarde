...
EXPECTED_CLI_SHA256 = "30481c1449b5a1869773d21b934789d42bb1d6a9cab8d81f873d50055af4e34c"
SAMPLES = [
    ...
    ("exceptional", "NullResourceExceptionalCore", "NullResourceExceptionalRunner"),
    ("same-type", "NullResourceCore", "NullResourceShapesRunner"),
    ("ordinary-only", "NullCatchOnly", "NullCatchOnlyRunner"),
]
...
def compile_and_run(
    ...
    support_paths = []
    for helper in out.glob("*.java"):
        if helper.name in {f"{class_name}.java", f"{runner_name}.java"}:
            continue
        support_dir = variant_work / "support"
        support_dir.mkdir(exist_ok=True)
        support = support_dir / helper.name
        support.write_text(f"package {package};\n\n" + helper.read_text() if package else helper.read_text())
        support_paths.append(str(support))

    class_status = record_command(
        out,
        f"{variant}-javac",
        ["javac", "--release", "8", "-g:none", "-d", str(classes), str(compile_source), *support_paths],
    )
