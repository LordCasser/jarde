...
R=Path('/Users/lordcasser/workspace/projects/jarde');W=Path('/tmp/jarde-type-shadow-audit-0608');O=R/'openspec/evidence/java-syntax-2026-09-22/type-name-shadowing';W.mkdir(exist_ok=True);O.mkdir(exist_ok=True)
S={
'arg0':'''public class arg0 {
...
k=W/'original/arg0.class';assert run(['javap','-c','-v','-p',str(k)],'javap.txt')==0;assert run(['java','-Xverify:all','-cp',str(W/'original'),'ShadowRunner'],'original.txt')==0
cli=R/'target/debug/jarde-cli';sha=hashlib.sha256(cli.read_bytes()).hexdigest();p=subprocess.run([str(cli),'class-source','--input',str(k),'--class','arg0','--policy','single-class','--release','8','--format','text'],capture_output=True,text=True,timeout=45);assert p.returncode==0
(O/'jarde.java.txt').write_text(p.stdout);(O/'jarde-report.txt').write_text(p.stderr);j=W/'jarde';j.mkdir(exist_ok=True);(j/'arg0.java').write_text(p.stdout)
...
s['jarde_javac']=run(['javac','--release','8','-d',str(j/'classes'),str(j/'arg0.java'),str(O/'ShadowOther.java'),str(O/'ShadowRunner.java')],'jarde-javac.log')
if s['jarde_javac']==0:
...
g=next((W/'jadx').rglob('arg0.java'));t=g.read_text();(O/'jadx.java.txt').write_text(t);package=next((v for v in t.splitlines()if v.startswith('package ')),'');sup=W/'support';sup.mkdir(exist_ok=True)
for n in ['ShadowOther','ShadowRunner']:(sup/(n+'.java')).write_text(package+'\n'+S[n]+'\n')
s['jadx_javac']=run(['javac','--release','8','-d',str(W/'jadx-classes'),str(g),str(sup/'ShadowOther.java'),str(sup/'ShadowRunner.java')],'jadx-javac.log')
if s['jadx_javac']==0:
