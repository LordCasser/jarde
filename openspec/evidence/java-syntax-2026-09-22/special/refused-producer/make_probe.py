...
methods=u2(p);p+=2
ref=next(i for i,v in cp.items() if v[0]==10 and cp[cp[v[2]][1]][1]=='privateHelper')
for _ in range(methods):
 start=p;name=cp[u2(p+2)][1];ac=u2(p+6);p+=8
 if name=='privateHelper':struct.pack_into('>H',b,start,(u2(start)&~2)|1)
 for _ in range(ac):
  attr=cp[u2(p)][1];length=u4(p+2)
  if name=='superWithSideEffect' and attr=='Code':
   code=p+14;assert b[code+4]==0xb7;struct.pack_into('>H',b,code+5,ref)
  p+=6+length
out=Path('/tmp/jarde-special-retention');out.mkdir(exist_ok=True);(out/'SpecialProbe.class').write_bytes(b)
