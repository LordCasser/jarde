...
EVIDENCE = ROOT / 'openspec/evidence/java-syntax-2026-09-22/generic-method-references/fixture/root'
SOURCES = sorted((EVIDENCE / "inputs").glob("*.java"))
TARGET = EVIDENCE / "inputs/LambdaAdaptationProbe.java"
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="jarde-lambda-adaptation-fixture-") as temp:
        ...
        assert original_run.returncode == 0 and jadx_run is not None and jadx_run.returncode == 0
        assert original_lines == jadx_lines, "complete JADX behavior differs"
        cases = []
        for case, kind in (
            ...
            ("receiverNull", "receiver/constructor dynamic checks and call count"),
            ("receiverNullWrongArg", "receiver/constructor dynamic checks and call count"),
            ("receiverWrong", "receiver/constructor dynamic checks and call count"),
            ("receiverWrongNullArg", "receiver/constructor dynamic checks and call count"),
            ("receiverValidNullArg", "receiver/constructor dynamic checks and call count"),
            ("receiverValidWrongArg", "receiver/constructor dynamic checks and call count"),
            ("constructorText", "receiver/constructor dynamic checks and call count"),
            ("constructorNull", "receiver/constructor dynamic checks and call count"),
            ("constructorWrong", "receiver/constructor dynamic checks and call count"),
        ...
        cli_end = hashlib.sha256(cli_path.read_bytes()).hexdigest()
        assert cli_start == cli_end, "CLI changed during audit"
        summary = {
            "case_count": len(original_lines),
